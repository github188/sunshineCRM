...
$action_type = $action_array[0];//执行修改时=edit  by cwf
$mark = $action_array[1];//执行修改时=default  by cwf
$action_check = $action_array[0] . "_" . $action_array[1];//执行修改时=edit_default  by cwf
...
switch ($action_type) {
    ...
    case 'edit':
        ...
        require_once('newai.php');
        $fields['form']['action'] = FormPageAction("action", $_GET['action'] . "_data", $primarykey_index, $_GET[$primarykey_index]);//将字符串转化成加密字符串 by cwf
        newaiadd('edit');//此函数定义在newai_view.php中 by cwf
        break;
    case 'view':
        $tablename = $file_ini[$action]['tablename'];
        $SYTEM_CONFIG_TABLE != "" ? $tablename = $SYTEM_CONFIG_TABLE : '';
...
//        var_dump($returnsystemprivateconfig);//by cwf
        ...
        page_css($IE_TITLE);
        require_once('newai.php');
//        echo 'table befor';//by cwf
        newaiinit($fields, $mode);//获取表格及数据库数据，此函数在newai_init.php中定义
//        echo 'table after';//by cwf
        //在显示弹出明细窗口时下面出现的关闭按钮
