	...
	if(key!=kword.value && kword.value!="")
	{
     key=kword.value;
	   ...
	   if($('search_icon').src.indexOf("/images/quicksearch.gif")>=0)
	   {
	   	   ...
	   	   $('search_icon').onclick=function(){kword.value='按停车场名称搜索...';$('search_icon').src="<?php echo ROOT_DIR?>images/quicksearch.gif";$('search_icon').title="";$('search_icon').onclick=null;};
	   }
  }
...
<div style="border:1px solid #000000;margin-left:2px;background:#FFFFFF;">
  <input type="text" id="kword" name="kword" value="按停车场名称搜索..." onfocus="ctroltime=setTimeout(CheckSend,100);" onblur="clearTimeout(ctroltime);if(this.value=='')this.value='按停车场名称搜索...';" class="SmallInput" style="border:0px; color:#A0A0A0;width:145px;"><img id="search_icon" src="../../../Framework/images/quicksearch.gif" align=absmiddle style="cursor:pointer;">
</div>
...
print "
	...
	<td align=center >停车场名称列表</td>
	</tr>
	";

$sql = "select * from wu_tingchechang order by 停车场名称";
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	print "
	<tr class=TableData>
	<td align=center onclick=\"javascript:parent.user.location='user.php?停车场名称=$停车场名称&TO_ID=".$_GET['TO_ID']."&TO_NAME=".$_GET['TO_NAME']."&FORM_NAME=".$_GET['FORM_NAME']."';\" style=\"cursor:pointer\">".$停车场名称."</td>
	</tr>
	";
}
?>
