...
var ctroltime=null,key="";seluser="";olduser="";
function CheckSend()
{
	var kword=$("kword");
  ...
  if(kword.value=="" && $('search_icon').src.indexOf("../../../Framework/images/quicksearch.gif")==-1)
	...
	if(key!=kword.value ||  seluser!=olduser)
	{
     key=kword.value;
     olduser=seluser;
	   parent.user.location="user.php?action=SEARCH&所属用户="+seluser+"&TO_ID=<?php echo $_GET['TO_ID']?>&TO_NAME=<?php echo $_GET['TO_NAME']?>&FORM_NAME=<?php echo $_GET['FORM_NAME']?>&KEYVALUE="+kword.value;
	   if($('search_icon').src.indexOf("../../../Framework/images/quicksearch.gif")>=0)
	   {
	   	   ...
	   	   $('search_icon').onclick=function(){kword.value='按客户名称、拼音码搜索...';$('search_icon').src="../../../Framework/images/quicksearch.gif";$('search_icon').title="";$('search_icon').onclick=null;};
	   }
  }
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	print "
	<tr class=TableData>
	<td align=center onclick=\"javascript:seluser='".$rs_a[$i]['USER_ID']."';CheckSend();\" style=\"cursor:pointer\">".$所属用户."</td>
	</tr>
	";
}
?>
