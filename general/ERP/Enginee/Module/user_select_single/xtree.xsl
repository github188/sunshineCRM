...
<xsl:if test="@op_sms">
	<xsl:attribute name="op_sms"><xsl:value-of select="@op_sms"/></xsl:attribute>
</xsl:if>
<xsl:choose>
  <xsl:when test=".[TreeNode or @Xml]">
    <img type="img" align="absmiddle">
    <xsl:attribute name="src">../../../Framework/images/collapsed.gif</xsl:attribute>
    </img>
  </xsl:when>
  <xsl:otherwise>
    <img type="img" align="absmiddle" border="0">
    <xsl:attribute name="src">../../../Framework/images/transparent.gif</xsl:attribute>
    </img>
  </xsl:otherwise>
</xsl:choose>
