...
while (!$rs->EOF)
{
    ...
    if ( $OVER_STATUS == "0" )
    {
        $CONTENT .= " <a href=\"calendar_status.php?CAL_ID=".$CAL_ID."&OVER_STATUS=1&YEAR={$YEAR}&MONTH={$MONTH}&DAY={$DAY}&GOBACK=calendar_day\">完成</a>";
    }
    else if ( $OVER_STATUS == "" || $OVER_STATUS == "1" )
    {
        $CONTENT .= " <a href=\"calendar_status.php?CAL_ID=".$CAL_ID."&OVER_STATUS=0&YEAR={$YEAR}&MONTH={$MONTH}&DAY={$DAY}&GOBACK=calendar_day\">未完成</a>";
    }
