	...
	*/
	...
	GLOBAL
		...
		if (!isset($ADODB_CACHE_DIR)) {
			$ADODB_CACHE_DIR =ROOT_DIR.'general/ERP/cache'; //(isset($_ENV['TMP'])) ? $_ENV['TMP'] : '/tmp';
		} else {
...
*/
	...
	var $leftOuter = true; /// operator to use for left outer join in WHERE clause
	var $rightOuter = true; /// operator to use for right outer join in WHERE clause
...
*/
	...
	global $ADODB_FETCH_MODE;
		...
		if ($this->metaTablesSQL) {
			...
			if ($this->fetchMode !== false) $savem = $this->SetFetchMode(false);
			//print $this->metaTablesSQL;exit;
			$rs = $this->CacheExecute(3600*24,$this->metaTablesSQL);
			if (isset($savem)) $this->SetFetchMode($savem);
			$ADODB_FETCH_MODE = $save;
	...
	global $ADODB_FETCH_MODE;
		...
		if (!empty($this->metaColumnsSQL)) {
			...
			$ADODB_FETCH_MODE = ADODB_FETCH_NUM;
			if ($this->fetchMode !== false) $savem = $this->SetFetchMode(false);
			$rs = $this->CacheExecute(5,sprintf($this->metaColumnsSQL,($normalize)?strtoupper($table):$table));
			if (isset($savem)) $this->SetFetchMode($savem);
			$ADODB_FETCH_MODE = $save;
