...
$RemoteHostName = $_GET['RemoteHostName'];
$RemoteHostNameURL = "http://".$RemoteHostName."/hostlist2009.php";
...
for($i=0;$i<sizeof($file);$i++)                {
    ...
    $ElementArray = explode('||',$file[$i]);
    //分离主机名
    $hostName = $ElementArray[0];        //print "主机名：".$hostName."<BR>";
    //分离下载的文件名
    ...
    if($FILE_EXEC=="1")                        {
        ...
        if($PacketNameUpdate[$fileName]=="1")        {
            $ReDownload = "<a href=?".base64_encode("action=ExecRemoteFileServer&FileName=$fileName&RemoteHostName=".$_GET['RemoteHostName']."")."><font color=003399 title='已安装,点击重新安装'>已安装,点击重新安装</font></a>";
        }
        else    {
			if($INSTALL_PACKET_NAME==$fileName)		{
				$ReDownload = "<a href=?".base64_encode("action=ExecRemoteFileServer&FileName=$fileName&RemoteHostName=".$_GET['RemoteHostName']."")."><font color=red>点击安装更新包文件</font></a>";
			}
			else			{
        ...
        $TargetFtpServer = $_GET['RemoteHostName'];
        $DefaultFtpServer = $SERVER_NAME;
        $ReDownload = "<a href=?".base64_encode("action=downloadRemoteFile&FileName=$fileName&RemoteHostName=".$_GET['RemoteHostName']."")."><font color=green>点击下载更新包文件HTTP</font></a>";

        //$ReExec = '';
    }
    //print "远程文件大小：$RemoteSize<BR>";
    //print "本地文件大小：$LocalSize<BR>";
    $text .= "<tr height=25><td align='middle'>更新包文件：$fileName</td> <td align='left'>大小：".$RemoteSize."B</td> <td align='left' nowrap><div id=ElementStatus".$i."></div></td><td align='left'><div id=ElementReDownload".$i."></div></td></tr>\n";
...
tableFilterContent($text,600);
    ...
    print "<a href='http://www.tongda2000.com/edu' target=_blank><font color=green>通达OA教育管理组件 http://www.tongda2000.com/edu</font></a>";
...
//调用远程服务器上面的程序执行解压操作
if($_GET['action']=="ExecRemoteFileServer"&&$_GET['FileName']!=""&&$_GET['RemoteHostName']!="")        {
    $dir = '.';                //文件地址所在
    $key = '../';            //解压文件目录
    ...
    if(file_exists($key.$FileName.".sql"))    {
        ...
        for($i=0;$i<sizeof($FileArray);$i++)        {
			...
			if($sql=="SET USER PRIV ALL")		{
				$sql = "select FUNC_ID from sys_function";
				$rs = $db->Execute($sql);
				$rs_a = $rs->GetArray();
				$NewText = '';
				...
				$sql = "update user set FUNC_ID_STR='$NewText' where USER_ID='admin'";
				$rs = $db->Execute($sql);
				//print $sql;exit;
			}
			else	{
