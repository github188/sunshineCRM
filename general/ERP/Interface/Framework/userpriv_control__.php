...
function element_input( $showtext, $showpic, $name, $value, $ischecked )
{
				if ( $ischecked )
				{
								print "<input type=\"checkbox\" name=\"{$name}\" value=\"{$value}\" checked><img src=\"../../Framework/images/menu/{$showpic}\" width=17 height=17>&nbsp;&nbsp;{$showtext}\n";
				}
				else
				{
								print "<input type=\"checkbox\" name=\"{$name}\" value=\"{$value}\"><img src=\"../../Framework/images/menu/{$showpic}\" width=17 height=17>&nbsp;&nbsp;{$showtext}\n";
				}
}
...
function element_header( $showtext, $showpic, $name, $value )
{
				print "<tr title=\"{$showtext}\">\n";
				print "<td nowrap>\n";
				print "<img src=\"../../Framework/images/menu/{$showpic}\" width=17 height=17>&nbsp;&nbsp;{$showtext}\n";
				print $showtext_add;
				print "</td>\n";
				print "</tr>\n";
}
...
echo "<s";
echo "cript> var MENU_ID_ARRAY = new Array(); ";
$i = 0;
...
echo " function check_all(menu_all,MENU_ID) {   for (i=0;i<document.all(MENU_ID).length;i++)  {    if(menu_all.checked)       document.all(MENU_ID).item(i).checked=true;    else       document.all(MENU_ID).item(i).checked=false;  }   if(i==0)  {    if(menu_all.checked)       document.all(MENU_ID).checked=true;    else       document.all(MENU_ID).checked=false;  } }  function mysubmit";
echo "() {   func_id_str=\"\";      for(j=0;j";
echo "<";
echo sizeof( $rs_array_menu );
echo ";j++)   {     MENU_ID=MENU_ID_ARRAY[j];      for(i=0;i<document.all(MENU_ID).length;i++)     {         el=document.all(MENU_ID).item(i);         if(el.checked)         {  val=el.value;            func_id_str+=val + \",\";         }     }          if(i==0)     {         el=document.all(MENU_ID);         if(el.checked)         {  val=el.value;            func_id_str+=val + \",\";      ";
echo "   }     }   }   //document.";
echo $var;
echo ".value = func_id_str ;    ";
print "location=\"?action=edit_purview_data&FUNC_ID_STR=\"+ func_id_str +\"&USER_PRIV=".$_GET['USER_PRIV']."\";\n";
print "}\n";
echo "</script> </head> <table border=\"0\"> <tr> <td valign=\"top\" class=TableData> ";
