...
global $db;
$GLOBALS['_GET']['USER_PRIV'] = isset( $_GET['USER_PRIV'] ) ? $_GET['USER_PRIV'] : 3;
...
foreach ( $rs_array_menu as $list_menu )
{
				...
				foreach ( $menu['index']["".$list_menu['MENU_ID'].""] as $menu_list )
				{
								...
								if ( is_array( $menu_2 ) )
								{
												...
												if($systemfuncid=="") $ischecked = 0;
												//print $ischecked."ddddddddddd";
												//print_R($systemfuncid);
												element_title( $menu['index_name']['FUNC_NAME'][$menu_list], $pic, $list_menu['MENU_ID'], $menu['index_name']['FUNC_ID'][$menu_list], $ischecked );
												unset( $ischecked );
												foreach ( $menu_2 as $list )
												{
																$systemfuncid = $menu['index_name']['FUNC_ID'][$list];
																$ischecked = in_array( $systemfuncid, $FUNC_ID_STR_ARRAY );
																if($systemfuncid=="") $ischecked = 0;
																$pic_array = explode( "/", $menu['index_name']['FUNC_CODE'][$list] );
																$pic = $pic_array[0].".gif";
																element_tr( $menu['index_name']['FUNC_NAME'][$list], $pic, $list_menu['MENU_ID'], $menu['index_name']['FUNC_ID'][$list], $ischecked );
																unset( $ischecked );
												}
								}
								else
