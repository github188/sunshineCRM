...
$goalfile = "sellcontract_print_config.ini";
...
if ( $_GET['action'] == "" || $_GET['action'] == "init" )
{
	...
	// 销售单打印字段配置
	$sellplanmain_columns=returntablecolumn('v_sellcontract');
	@$sellplanmain_ini_file = @parse_ini_file('../JXC/Model/sellcontract_newai.ini',true);
	...
	global $db;
	$sql = "select fieldname,chinese from systemlang where tablename='v_sellcontract'";
	$rs=$db->Execute($sql);
	$rs_a = $rs->GetArray();
	foreach ($rs_a as $row){
	...
	print_title("销售合同附加附加条款");
	$str = $ini_file['fujia']['con'];
	$str = str_replace("^^","\r\n",$str);
		print('<tr>
				<td class="TableData" nowrap="" colspan="3">
					<textarea class="BigInput" name="fujia" title="销售合同附加内容" wrap="yes" rows="10" cols="60">'.$str.'</textarea>				
				</td>
			</tr>');
