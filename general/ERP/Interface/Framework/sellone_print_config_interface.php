...
if ( $_GET['action'] == "" || $_GET['action'] == "init" )
{
	...
	foreach ($pro_printfieldlist as $key=>$val){
		...
		print('<tr>
				...
				</td><td class="TableData" nowrap="" colspan="1"><input type="text" title="顺序"  maxlength="20" size="5" name="selldetailfiledorder['.$key.']" value="'.$sell_order_detail_print_field[$key].'" '.$checked.'>
				</td>
			</tr>');
	}

	print_title("店面销售单附加附加条款");
	$str = $ini_file['fujia']['con'];
	$str = str_replace("^^","\r\n",$str);
		print('<tr>
				<td class="TableData" nowrap="" colspan="3">
					<textarea class="BigInput" name="fujia" title="店面销售单打印附加内容" wrap="yes" rows="10" cols="50">'.$str.'</textarea>				
				</td>
			</tr>');
