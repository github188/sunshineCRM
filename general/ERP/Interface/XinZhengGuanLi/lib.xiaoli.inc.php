...
//$sql = "update edu_banji set 是否毕业标记='0' where 毕业时间>='".date('Y-m-d')."'";
//$db->Execute($sql);
...
function 处理听课教师用户名为空的情况()			{
	global $db;
	$sql = "select distinct 听课教师姓名 from edu_tingke where 听课教师工号=''";
	$rs  = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	for($i=0;$i<sizeof($rs_a);$i++)				{
		$听课教师姓名 = $rs_a[$i]['听课教师姓名'];
		$USER_ID = returntablefield('user',"USER_NAME",$听课教师姓名,"USER_ID");
		$sql = "update edu_tingke set 听课教师工号='$USER_ID' where 听课教师姓名='$听课教师姓名'";
		$db->Execute($sql);
		//print $sql."<BR>";
	}

	$sql = "select distinct 被听教师姓名 from edu_tingke where 被听教师工号=''";
	$rs  = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	for($i=0;$i<sizeof($rs_a);$i++)				{
		$被听教师姓名 = $rs_a[$i]['被听教师姓名'];
		$USER_ID = returntablefield('user',"USER_NAME",$被听教师姓名,"USER_ID");
		$sql = "update edu_tingke set 被听教师工号='$USER_ID' where 被听教师姓名='$被听教师姓名'";
		$db->Execute($sql);
		//print $sql."<BR>";
	}
}
...
function returnWeekDayNumber($周次,$学期名称='')		{
...
function returnWeekDayNumberALL($周次,$学期名称='')		{
	global $db,$开始时间,$结束时间,$开始时间Array,$结束时间Array,$ShowData,$ShowDataText;
	if($学期名称=='')		{
		$sql = "select 结束时间,开始时间 from edu_xueqiexec where 当前学期='1'";
	}
...
function XiaoLiArray()		{
	...
	$sql = "select * from edu_schoolcalendar where 学期='$CurXueQi'  and 节假日!='开学'";
	$rs = $db->CacheExecute(15,$sql);
	$rs_a = $rs->GetArray();
	for($i=0;$i<sizeof($rs_a);$i++)				{
		$开始时间S = $rs_a[$i]['开始时间'];
...
function ReturnTeacherSchedule($学期,$教师用户名,$教师,$周次)  {
	global $db,$CurXueQi;
	$sql = "select 星期,节次,教室,课程,班级,单双周 from edu_schedule where 学期='$学期' and 教师用户名='$教师用户名' order by 星期 asc,单双周 asc,节次 asc,班级 asc,课程 asc";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	//过滤合班上课
	for($i=0;$i<sizeof($rs_a);$i++)		{
...
function Banji_ShiXi_JiaoXueJinCheng($班级,$周次,$学期,$考勤日期)		{
	...
	//判断是否是合班上课的情况 2011-10-24增加此功能
	$班级ARRAY_TEMP  = explode('::',$班级);
	//print_R($班级ARRAY_TEMP);exit;
	for($XXX=0;$XXX<sizeof($班级ARRAY_TEMP);$XXX++)					{
		if($班级ARRAY_TEMP[$XXX]!="")							{
			$班级	= $班级ARRAY_TEMP[$XXX];
			...
			if($内容!="")		{
				...
				*/
				$sql = "delete from edu_teacherkaoqinmingxi where 班级='$班级' and 学期='$学期' and 周次='$周次' and
							( 节次 in ('1-2','3-4','5-6','7-8') or 节次 in ('1','2','3','4','5','6','7','8') )
							and 考勤日期='$考勤日期'";
				$db->Execute($sql);
...
function Teacher_TiaoKe_Infor($教师,$教师用户名,$上课时间,$节次,$课程,$学期)		{
	...
	$sql = "select * from edu_scheduletiaoke
				where 教师用户名='$教师用户名'
				and
				(	(原上课时间='$上课时间' and 原节次='$节次')
					or
					(新上课时间='$上课时间' and 新节次='$节次')
				)
				and 审核状态='1'";
	//print $sql."教师调课过滤<BR>";
	$rs = $db->Execute($sql);
	...
	if($编号!="")		{
		//print $sql."调课表<BR>";
		$sql = "delete from edu_teacherkaoqinmingxi where 学期='$学期' and 教师用户名='$教师用户名' and 课程='$课程' and 考勤日期='$原上课时间' and 节次='$原节次'";
		$db->Execute($sql);
		global $SHOWTEXT; if($SHOWTEXT&&sizeof($rs_a)>0)print "教师调课过滤<BR><font color=red><B>$sql</B></font><BR>";
		global $SHOWTEXT; if($SHOWTEXT&&sizeof($rs_a)>0)print_R($rs_a);;
		//存在记录,重新较验新的记录是否存在
		$sql = "select * from edu_teacherkaoqinmingxi where 学期='$学期' and 教师用户名='$教师用户名' and 课程='$课程' and 考勤日期='$新上课时间' and 节次='$新节次'";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
		$编号X = $rs_a[0]['编号'];
		if($编号X=="")		{
			...
			$X = 7-$当前星期数;
			$安排内容 = returnJieCiName($节次Array[0]);
			$有效时间 = returnJieCiTime($安排内容,$当前星期数);
			$有效时间Array = explode('-',$有效时间);
			$有效时间0Array = explode(':',$有效时间Array[0]);
			$有效时间1Array = explode(':',$有效时间Array[1]);
			global $SYSTEM_MERGE_CLASSTABLE;
			if($节次Array[1]!=""&&$SYSTEM_MERGE_CLASSTABLE = "1")		{
				...
				$有效时间1 = returnJieCiTime($安排内容1,$当前星期数);
...
function   Init_Teacher_KaoQin_Data($节假日调课_调整后时间,$默认教师名称,$教师用户名)							{
	...
	//对校历表进行过滤
	$sql = "select 节假日 from edu_schoolcalendar where 开始时间<='$节假日调课_调整后时间' and 结束时间>='$节假日调课_调整后时间'  and 节假日!='开学'";
	$rs = $db->Execute($sql);
	$节假日 = $rs->fields['节假日'];
	if($节假日!="")				{
		//当前时间为放假时间,系统不增加考勤信息
		...
		//for($ii=1;$ii<=$DayIndexMonth;$ii++)			{
			...
			for($iii=0;$iii<sizeof($JieCiArray);$iii++)		{
				...
				if($WeekDaySchedule[$第二节次][$单双周]['课程']==$WeekDaySchedule[$节次][$单双周]['课程']
					...
					$有效时间22 = returnJieCiTime($安排内容,$当前星期数);
				...
				if($课程!=""&&
					...
					for($XXX=0;$XXX<sizeof($班级ARRAY_TEMP);$XXX++)					{
						if($班级ARRAY_TEMP[$XXX]!="")							{
							$Element['班级']	= $班级ARRAY_TEMP[$XXX];
							//重新回归以前代码 2011-10-24增加注释
							$ElementValue = array_values($Element);
							$sqlValueText = "'".join("','",$ElementValue)."'";
							$ElementName = array_keys($Element);
							$sqlNameText = "`".join("`,`",$ElementName)."`";
							//判断记录是否存在,如果存在则不进行插入操作
							$sql = "select COUNT(*) AS NUM from edu_teacherkaoqinmingxi where 教师姓名 ='$真实姓名' and 教师用户名 ='$教师用户名' and 考勤日期='$节假日调课_调整后时间' and 节次='".$Element['节次']."' and 课程='$课程' and 班级='".$Element['班级']."' ";
							$rs = $db->Execute($sql);
...
function returnJieCiTime($安排内容,$星期=0)  {
	switch($星期)		{
		case 0:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周日");
			break;
		case 1:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周一");
			break;
		case 2:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周二");
			break;
		case 3:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周三");
			break;
		case 4:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周四");
			break;
		case 5:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周五");
			break;
		case 6:
			$时间段 = returntablefield("edu_timesetting","安排内容",$安排内容,"周六");
			break;
	}
	//print $星期;
	//print $时间段;exit;
