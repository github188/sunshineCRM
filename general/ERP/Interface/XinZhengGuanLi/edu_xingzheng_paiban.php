...
<?php
	...
	print "</td></tr>";
	print "<tr class=TableData><td>";
	print "<font color=gray>注意:调整班次后,系统自动会把从明天开始的未执行的行政考勤记录清空,新生成的考勤明细数据系统会从系统定时任务中生成,你也可以在初始化中进行生成.</font>";
	print "</td></tr>";
	$考勤日期 = 返回目标日期($当前周次,$当前星期);
	?>
<input type="hidden" name="考勤日期" value=<?php echo $考勤日期;?> >
...
<?php
	...
	$当前周次 = $_GET['当前周次'];
	$上周周次 = $当前周次-1;
	$sql = "select * from edu_xingzheng_paiban where 周次='$上周周次' and 学期名称='$当前学期'";
	$rs = $db -> Execute($sql);
	$rs_a = $rs -> GetArray();
	//本周
	$sql = "select * from edu_xingzheng_paiban where 周次='$当前周次' and 学期名称='$当前学期'";
	$rs = $db -> Execute($sql);
	$rs_a本周 = $rs -> GetArray();
	...
	$前周周次 = $当前周次-2;
	$sql = "select * from edu_xingzheng_paiban where 周次='$前周周次' and 学期名称='$当前学期'";
	$rs = $db -> Execute($sql);
	$rs_a = $rs -> GetArray();
...
<input name='从上周获取数据' value='从第<?php echo ($当前周次-2)?>周获取排班数据' class=SmallButton type=Button OnClick="location='?当前周次=<?php echo $当前周次?>&action=从前周获取数据'" <?php echo $disabled_前周?>>
</td>
</tr>
...
require_once('../Help/module_xingzhengkaoqin_paiban.php');
	...
	{
		global $db;
		$sql = "select 班次名称,考勤时间段 from edu_xingzheng_banci";
		$rs = $db -> Execute($sql);
		$rs_a = $rs -> GetArray();
		if(!is_array($rs_a))
		$rs_a = array();
	...
	function 排班时自动清空今天以后的排班数据()				{
		global $db;
		$date1 = date("Y-m-d",mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')));
		$sql = "delete from edu_xingzheng_kaoqinmingxi where 日期>='$date1'";
		$db->Execute($sql);
		//print $sql;
	}
?>
