...
if($_GET['pageAction']=="ExportDataToFile")				{
	...
	header('Content-Disposition: attachment;filename=行政考勤统计[人员:'.$_GET['LOGIN_USER_NAME'].']['.$TEXT.']['.date("Y-m-d-H").'].xls');
...
&nbsp;<INPUT TYPE=\"button\" VALUE=\"导出\" class=SmallButton ONCLICK=\"location='?pageAction=ExportDataToFile&action=".$_GET['action']."&开始周=".$_GET['开始周']."&结束周=".$_GET['结束周']."&Month=".$_GET['Month']."&LOGIN_USER_NAME=".$_SESSION['LOGIN_USER_NAME']."'\">
...
//print_R($Header);
print "<tr class=TableHeader><td colspan=1>人员</td>";
...
$sql = "select distinct 人员 from edu_xingzheng_kaoqinmingxi where 人员='".$_GET['LOGIN_USER_NAME']."' order by 人员";
$rs = $db->CacheExecute(150,$sql);
$rs_a = $rs->GetArray();
...
for($i=0;$i<sizeof($rs_a);$i++)						{
	$Element = $rs_a[$i];
	$人员  = $Element['人员'];
	$Text = $returnTeacherKaoQin['人员'][$人员];
	//print_R($Text);
	//总需考勤次数为0时表示没有该行政人员的课程信息,所以在此不用显示出来
	//if($Text['总需考勤次数']>0)				{
		//print_R($Header);
		print "<tr class=TableData><td colspan=1>$人员</td>";
		for($iX=0;$iX<sizeof($Header);$iX++)						{
			$状态 = $Header[$iX];
			print "<td colspan=1>&nbsp;<a href=\"?action=StaticSingleTeacher&开始时间=$开始时间&结束时间=$结束时间&人员=$人员&TYPE=$状态\" target=_blank>".$Text[$状态]."</a></td>";
		}
		print "</tr>";
	//}
}
...
//数据明细
if($_GET['action']=="StaticSingleTeacher"&&$_GET['开始时间']!=""&&$_GET['结束时间']!=""&&$_GET['人员']!="")			{
$DateMonth = $_GET['Month'];
$人员 = $_GET['人员'];
$TYPE = $_GET['TYPE'];
...
<table width=100% align=center class=TableBlock>
<tr class=TableHeader>
<td colspan=".sizeof($Column)."+3>行政人员打卡考勤管理(统计时间:".$开始时间." 至 ".$结束时间." 行政人员姓名:$人员 统计项:$TYPE)[上班和下班按两次进行统计]&nbsp;<input type=button value=\"关 闭\" class=\"SmallButton\" onclick=\"window.close();\"></td>
</tr>
<tr class=TableHeader>";
...
$sql = "select * from edu_xingzheng_kaoqinmingxi where 人员='$人员' and 日期 >='$开始时间' and 日期<='$结束时间' $AddSql order by 日期,班次";
$rs = $db->CacheExecute(150,$sql);
$rs_a = $rs->GetArray();
//print_R($sql);
for($i=0;$i<sizeof($rs_a);$i++)						{
...
if($_GET['action']=="init_default"||$_GET['action']=="")					{
	...
	print "
		...
		<tr class=TableHeader><td valign=bottom align=left>行政人员卡机考勤管理 ".$html_etc['edu_xingzheng_kaoqinmingxi']['学期'].":$学期名称</td></tr>
		...
		print "
		<table border=0 class=TableBlock width=100% >
		<tr class=TableHeader><td valign=bottom align=left>行政人员卡机考勤按周次查询 ".$html_etc['edu_xingzheng_kaoqinmingxi']['学期'].":$学期名称</td></tr>
		<tr class=TableData><td valign=bottom align=left>
		<BR>";
...
function returnTeacherKaoQin($开始时间='',$结束时间='')		{
	...
	$sql = "select COUNT(*) AS NUM,上班考勤状态,人员 from edu_xingzheng_kaoqinmingxi where 日期 >='$开始时间' and 日期<='$结束时间'  group by 上班考勤状态,人员";
	//print $sql;exit;
	$rs = $db->CacheExecute(150,$sql);
	$rs_a = $rs->GetArray();
	//print_R($rs_a);
	...
	for($i=0;$i<sizeof($rs_a);$i++)						{
		...
		if($上班考勤状态!="")		{
			$NewArray['人员'][$人员][$上班考勤状态] += $NUM;
			$NewArray['状态'][$上班考勤状态] += $NUM;
		}
	}

	$sql = "select COUNT(*) AS NUM,下班考勤状态,人员 from edu_xingzheng_kaoqinmingxi where 日期 >='$开始时间' and 日期<='$结束时间' group by 下班考勤状态,人员";
	//print $sql;exit;
	$rs = $db->CacheExecute(150,$sql);
	$rs_a = $rs->GetArray();
	//print_R($rs_a);
