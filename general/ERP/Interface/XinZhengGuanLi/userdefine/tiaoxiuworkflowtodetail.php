...
//#########################################################
$tiaoxiuworkflowtodetail = "工作流ID号到工作流明细页面";
//#########################################################
function tiaoxiuworkflowtodetail_Value($fieldvalue,$fields,$i)		{
	global $db;
	...
	{	
		...
		else if($调休审核人 != "" )
		{
			if($调休审核状态 == "是")
			{
				$工作流ID号 = "<a><font color = red>调休申请通过</font></a>";
			}
			else if($补班审核状态!="是")
			{
				$工作流ID号 = "<a><font color = gray>补班申请成功</font></a>";
			}
			else
			{
				$工作流ID号 = "<a><font color = gray>调休申请未通过</font></a>";
			}	
