...
//#########################################################
$buxiuworkflowtodetail = "工作流ID号到工作流明细页面";
//#########################################################
function buxiuworkflowtodetail_Value($fieldvalue,$fields,$i)		{
	...
	{	
		...
		else if($补休审核人 != "" ) 
		{
			if($补休审核状态 == "是")
			{
				$工作流ID号 = "<a><font color = red>补休申请通过</font></a>";
			}
			
			else
			{
				$工作流ID号 = "<a><font color = gray>补休申请未通过</font></a>";
			}


			
		}
		else if($补休审核人 == ""&&$加班审核人 != "" &&$补休审核状态 == "否" && $加班审核状态 == "是")
		{
				$工作流ID号 = "<a href=\"?action=buxiuaction&编号=$编号\"><font color = blue>申请补休</font></a>";
		}
	}
	return $工作流ID号;
}
