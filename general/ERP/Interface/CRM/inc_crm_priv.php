...
$sql = "select distinct DEPT_NAME AS 所属部门  from department order by DEPT_NO";
$rs = $db->Execute($sql);
$rsX_a = $rs->GetArray();
...
if($_GET['FileNameSELF']!="")						{
	...
	$sql = "select * from ".$MYSQL_DB.".systemprivateinc where `FILE`='".$_GET['FileName']."' and `MODULE`='".$_GET['ModuleName']."'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
...
if($_GET['action']=='DataDeal')					{
	...
	$sql = "select MODULE from ".$MYSQL_DB.".systemprivateinc where `FILE`='$FILE' and `MODULE`='$MODULE'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	...
	//print $MODULE2;
	if($MODULE2!="")		{
		$sql = "update ".$MYSQL_DB.".systemprivateinc set `DEPT_ID`='$DEPT_ID',`USER_ID`='$USER_ID',`ROLE_ID`='$ROLE_ID',`DEPT_NAME`='$DEPT_NAME',`USER_NAME`='$USER_NAME',`ROLE_NAME`='$ROLE_NAME' where `FILE`='$FILE' and `MODULE`='$MODULE'";
	}
	else	{
		$sql = "insert into ".$MYSQL_DB.".systemprivateinc values('','$FILE','$MODULE','$DEPT_ID','$DEPT_NAME','$ROLE_ID','$ROLE_NAME','$USER_ID','$USER_NAME')";
	}
	$db->Execute($sql);
