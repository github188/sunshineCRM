<?php
    ...
    if($_GET['check'] == "upd"){
		    page_css("桌面参数设置");
			$DataText = "<?php \$桌面模块位置 = '".$_POST['mokuaiweizhi']."';\$模块显示条数 = '".$_POST['xianshitiaoshu']."';\$左右栏目宽度 = '".$_POST['lanmukuandu']."';\$上下滚动显示 = '".$_POST['shangxiagundong']."';\$展开折叠模块 = '".$_POST['zhankaizhedie']."';\$左侧栏目宽度 = '".$_POST['left_width']."';\$各模块显示行数 = '".$_POST['hanshu']."';\$列表上下滚动显示 = '".$_POST['gongdun']."';\$批量应用 = '".$_POST['hanshu_gundong_all']."';?>";	
			@!$handle = fopen($goalfile, 'w');
			fwrite($handle, $DataText);
	        fclose($handle);
			print "
			<div align=\"center\" title=\"桌面参数设置\">
			<table class=\"MessageBox\" align=\"center\" width=\"400\">
			<tr><td class=\"msg info\">
			<div class=\"content\" style=\"font-size:12pt\">&nbsp;&nbsp;设置参数已经保存!</div>
			</td></tr>
			</table><br>
			<div align=center>
			";
