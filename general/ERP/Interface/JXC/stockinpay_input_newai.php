...
if($_GET['action']=='dataDeal')		{
	require_once('dataDealStockInPay.php');
	//print_R($db);
	if($_POST['dataDealMethod']=="update")	{
		dataDeal_edit();
		print "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=stockinpay_newai.php'>\n";
	}
	else	{
		dataDeal_add();
...
if($_GET['action']=='edit_default'&&$_GET['ROWID']!="")		{
	$sql = "select * from paymain where ROWID='".$_GET['ROWID']."'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();	//print_R($rs_a);
	//tablecode,tabledate,supplyid,paymentid,banckid,checkid,checkdate,currency,paymoney,paymainstate,ROWID,fillindate,state,user_id,factgetamt,checkamt,fileaddress
	$tablecode = $rs_a[0]['tablecode'];
	$tabledate = $rs_a[0]['tabledate'];
	$supplyid = $rs_a[0]['supplyid'];
	$paymentid = $rs_a[0]['paymentid'];
	$banckid = $rs_a[0]['banckid'];
	$checkid = $rs_a[0]['checkid'];
	$checkdate = $rs_a[0]['checkdate'];
	$currency = $rs_a[0]['currency'];
	$paymoney = $rs_a[0]['paymoney'];
	$paymainstate = $rs_a[0]['paymainstate'];
	$ROWID = $rs_a[0]['ROWID'];
	...
	$sql = "select * from paydetail where mainrowid='".$_GET['ROWID']."'";
	$rs2 = $db->Execute($sql);
...
//alert("numdotValue="+numdotValue);
 ...
 var fieldNum2="";
  ...
  URL="./DataQuery/amtquery.php?moduleId="+moduleId+"&fieldname="+fieldname+"&fieldValue="+fieldValue+"&codeValue="+codeValue+"&countStr="+countStr+"&cliNo="+cliNo+"&supplyId="+supplyId;
  //alert(URL);
   loc_x=354;
   loc_y=262;
...
function check_query(field)
{
  ...
  URL="getcheckindexcaigou.php?fieldId="+fieldId+"&fieldRowId="+fieldRowId+"&ModuleId="+ModuleId+"&supplyid="+supplyid;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
...
<table width="95%" class="TableBlock">
  <tr>
    <td nowrap align="left" class="TableHeader" colspan="2">&nbsp;&nbsp;<b>采购付款管理      ：以下项目带*号的为必填项目</b></td>
   ...
   <tr>
       ...
       <td nowrap align="left" class="TableData">
           ...
           <input  readonly type="text" name="tablecode" value="<?php echo returnDateAutoIncrement("tablecode","paymain");?>" size="10"  maxlength="50"   class="SmallInput">
<b>*</b>
       </td>
...
<b>*</b>
      ...
      <?php
		print_select_single_select2("paymentid",'',"payment","code","name",$paymentid);
	?>
<b>*</b>
       </td>
...
<b>*</b>
 ...
 <input type="button" name="b1" value="核销" class="SmallButton" onclick="check_query('checkamt');"  value="<?php echo $checkamt?>">
       ...
       <td nowrap align="left" class="TableData">
           ...
           <input   type="text" name="paymainstate" size="10"  maxlength="50"   class="SmallInput"  value="<?php echo $paymainstate?>">
      ...
      <input type="hidden" name="Add_fieldName" value=",tablecode,tabledate,supplyid,paymentid,banckid,checkid,checkdate,currency,paymoney,factgetamt,checkamt,paymainstate">
      <input type="hidden" name="Add_fieldType" value=",CHAR,DATE,CHAR,CHAR,CHAR,CHAR,DATE,CHAR,MONEY,MONEY,MONEY,DATE">
      <input type="hidden" name="Add_table" value="paymain">
      <input type="hidden" name="Add_url" value="buypay/stockinpay/add.php">
      <input type="hidden" name="Back_url" value="buypay/stockinpay/index.php">
      <input type="hidden" name="FLD_MUST_STR" value="tablecode,tabledate,supplyid,paymentid,banckid,currency,paymoney,">
      <input type="hidden" name="FLD_NAME_STR" value="付款单编号,付款日期,供应商,付款方式,银行帐号,货币,总金额,">
...
<td nowrap width="100" align="center" class="TableContent"><b>入库单编号*</b></td>
...
<table width="95%" class="TableBlock">
 ...
 <td nowrap width="100" align="left" class="TableData">
            ...
            <input type="hidden" name="stockinid_id<?php echo $MarkName?>" id="id_stockinid_id_1_wxdid_<?php echo $MarkID?>">
            <input   type="text" id="id_stockinid_1_wxd_<?php echo $MarkID?>"   name="stockinid<?php echo $MarkName?>" size="10"  maxlength="50" onClick="dataClick('<?php echo $MarkID?>'); " class="SmallInput2" onkeypress="amtQuery('stockinid','S08',<?php echo $MarkID?>);" ondblclick="amtQuery('stockinid','S08',<?php echo $MarkID?>);">
...
<div id="hiddenId">
      ...
      <input type="hidden" name="secAdd_fieldName" value=",stockinid,allpaymoney,haspaymoney,thispaymoney,paystate">
      <input type="hidden" name="secAdd_fieldType" value=",CHAR,MONEY,MONEY,MONEY,DATE">
      <input type="hidden" name="secAdd_table" value="paydetail">
      <input type="hidden" name="secFLD_MUST_STR" value="stockinid,allpaymoney,haspaymoney,thispaymoney,">
      <input type="hidden" name="secFLD_NAME_STR" value="入库单编号,总金额,已付金额,本次付款金额,">
...
<input type="button" name="btnsave" id="btnsave" class="SmallButton" value="删除明细" onClick="javascript:clickDeleteRow() ;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <INPUT class=SmallButton id=close onclick="location='stockinpay_newai.php'" type=button value=返回 name=close>
 <br>
  本页面执行时间: <script>
 var now1 =new Date()
...
<table border="0" cellspacing="1" width="95%" class="small" bgcolor="#000000" cellpadding="3"  style="display:none" >
 ...
 <td nowrap width="100" align="left" class="TableData">
            ...
            <input type="hidden" name="stockinid_id_dxw" id="id_stockinid_id_1_wxdid_1">
            <input   type="text" id="id_stockinid_1_wxd_1"   name="stockinid_dxw" size="10"  maxlength="50"  onClick="dataClick('1'); "  class="SmallInput2" onkeypress="amtQuery('stockinid','S08',1);"  ondblclick="amtQuery('stockinid','S08',1);">
