...
//}
//print_r($list);exit;
$sql="SELECT ".$date_format." as name,sum(a.jine) as sum FROM feiyongrecord a WHERE a.kind=-1 and a.createtime>='".$start_time."' and a.createtime<='".$end_time."' GROUP BY name";
//exit($sql);
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();
foreach ($rs_a as $row){
...
chart_index.setDataXML("<graph bgcolor='e1f5ff' caption='月度费用开支走势图' subCaption='精确到百位（四舍五入）' numberPrefix='' formatNumberScale='1' decimalPrecision='2' baseFontSize='14' numberSuffix='万'><?php foreach ($list as $key=>$vo){echo "<set name='".$key."' value='".($vo/10000)."'/>";}?></graph>");
