<?php
	...
	*/
	...
	//数据表模型文件,对应Model目录下面的v_ribao_newai.ini文件
	//如果是需要复制此模块,则需要修改$parse_filename参数的值,然后对应到Model目录 新文件名_newai.ini文件
	//print_r($_GET);exit;
	if(empty($_GET['createtime_开始时间'])){
		$date =  date("Y-m-d");
	}else{
		$date =  date("Y-m-d",strtotime($_GET['createtime_开始时间']));
	}
	
	$SYSTEM_ADD_SQL = " and (DATE(`createtime`) = '".$date."')";
	//echo $SYSTEM_ADD_SQL;exit;
	
	$filetablename		=	'v_ribao';
	$parse_filename		=	'v_ribao';
