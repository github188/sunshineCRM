...
if($_GET['action']=="dataDeal")			{
	...
	$sql = "insert into stockoutmain (
		tablecode,tabledate,stockoutpersion,stockid,stockiplaceid,
		supplyid,amt,factpayamt,ROWID,user_id,
		state,outtype,chinaAmt,sellman,fillindate,
		payamt,stockoutsign,flowState
		)
		values (
		'$tablecode','$tabledate','$USER_NAME', '$stockid','',
		'$supplyid','$amt','$amt','','$USER_NAME',
		'$state','$outtype','$chinaAmt','$buyman','$sendDate',
		'','$stockoutsign','$flowState')";
	$db->Execute($sql);
	...
	for($i=1;$i<=$_POST['num'];$i++)		{
		...
		$mode = $_POST["6".$i];
		$plannum = $_POST["7".$i];
		$stockoutfactnum = $_POST["8".$i];
		$price = $_POST["9".$i];
		$sellplandetailsign = $_POST["10".$i];
		$ROWID = $_POST["ROWID_".$i];

		//同步库存数据。
		$stockinfactnum = $stockoutfactnum;
		...
		if($backnum!="")		{
			...
			$sql = "insert into stockoutdetail (
			productid,plannum,stockoutfactnum,price,money,
			mainrowid,standard,mode,ordercode,allnum,storenum
			) values (
			'$productid','$backnum','$stockoutfactnum', '$price','$money',
			'$Insert_ID','$standard','$mode','$ordercode','$allnum','$MyStockNum')";
			$db->Execute($sql);
			//print $sql."<HR>";

			//修改采购订单明细的数据状态为部分执行
			$update = "update sellplandetail set dataState ='1',stockinfactnum=stockinfactnum+'$backnum' where ROWID='$ROWID'";
			$db->Execute($update);
			//print $update."<HR>";
...
function checkNum(numNo)
{
    ...
    if (lastNum<parseFloat(numIn)){
     if(window.confirm("本次出库数量大于应出库数量,是否继续进行？")){
     //return false;
...
<tr align="center" >
      ...
      <td nowrap  class="TableHeader" >已出库数量</td>
...
<?php
$sql = "select sellplanmain.tablecode,
				department.DEPT_NAME,
				sellplanmain.tabledate,
				sellplandetail.productid,
				product.productname,
				sellplandetail.standard,
				sellplandetail.mode,
				sellplandetail.plannum,
				sellplandetail.stockinfactnum,
		...
		from sellplandetail,
				...
				department,
				product
		where sellplandetail.mainrowid=sellplanmain.ROWID
				and sellplanmain.stockid=department.DEPT_ID and
				sellplandetail.productid=product.productid   and
				sellplanmain.flowState='1'
				and ( sellplandetail.plannum>sellplandetail.stockinfactnum
				or sellplandetail.dataState!='2' )
				and sellplanmain.supplyid='".$_GET['supplyId']."'
...
<td   align="left"><input readonly  type="text"   name="9<?php echo $MarkID?>" value="<?php echo $Element['price']?>" size="7" maxlength="54"  class="SmallStatic"></td>
...
$All_stockoutfactnum += $Element['stockoutfactnum'];
...
<td class="TableHeader" align="left"><?php echo $All_stockoutfactnum?></td>

<td  class="TableHeader" align="right"><?php echo $All_price?></td>

<td class="TableHeader" align="left" >
	<input   type="text"   name="countNum" size="10" maxlength="54"   class="SmallInput">
	</td>
<td  class="TableHeader" align="left">
	<input   type="text"   name="countAmt" size="10" maxlength="54"   class="SmallInput">
	</td>
<td  class="TableHeader" align="left">
	<input   type="text"   name="countAllAmt" size="10" maxlength="54"   class="SmallInput">
	</td>
</tr>
</table>
...
<input type="hidden" name="outtype" value="<?php echo $_GET['outtype']?>">
<input type="hidden" name="sendDate" value="<?php echo $_GET['sendDate']?>">
<input type="hidden" name="stockoutsign" value="<?php echo $_GET['stockoutsign']?>">
