...
if($_GET['action']=="dataDeal")			{
	...
	$countAllAmt = $_POST['countAllAmt'];

	$factpayamt = ereg_replace(',','',$factpayamt);
	$factpayamt = (int)$factpayamt;

	$countAllAmt = ereg_replace(',','',$countAllAmt);
	$amt = (int)$countAllAmt;
	...
	for($i=1;$i<=$_POST['num'];$i++)		{
		...
		$price = $_POST["9".$i];
		$requireplandetailsign = $_POST["10".$i];
		$ROWID = $_POST["ROWID_".$i];

		//$ordercode = $_POST['ordercode'];

		//当时流转数据不为0时，转换采购计划明细到采购订单。
		if($backnum!="")		{
			$sql = "insert into stockindetail (
			productid,plannum,stockinfactnum,price,money,
			mainrowid,standard,mode,ordercode,allnum
			) values (
			'$productid','$backnum','$stockinfactnum', '$price','$money',
			'$Insert_ID','$standard','$mode','$ordercode','$backnum')";
			$db->Execute($sql);
			//print $sql."<HR>";

			//修改采购计划明细的数据状态为部分执行
			$update = "update requireplandetail set dataState ='1',stockinfactnum=stockinfactnum+'$backnum' where ROWID='$ROWID'";
			$db->Execute($update);
			//print $update."<HR>";

			//修改采购计划主表的数据状态为部分执行
			$MAINROWID = returntablefield("requireplandetail","ROWID",$ROWID,"mainrowid");
			$update = "update requireplanmain set dataState ='1' where ROWID='$MAINROWID'";
			$db->Execute($update);
			//print $update."<HR>";

			//判断主表里面的数据是否完全执行
			$udpate = "select distinct mainrowid from requireplandetail where (dataState!='2' and stockinfactnum>=plannum) and mainrowid='$MAINROWID'";
			$rs = $db->Execute($udpate);
			//print $udpate."<HR>";
			$rs_a = $rs->GetArray();	//print_R($rs_a);
			$mainrowid = $rs_a[0]['mainrowid'];
			if($mainrowid!="")		{
				$update = "update requireplanmain set dataState ='2' where ROWID='$MAINROWID'";
				$db->Execute($update);
				//print $udpate."<HR>";
			}
		}
	...
	//如果采购计划明细部分数据流转完成，更新dataState为2。部分流转显示未全部执行，更新为1
	$udpate = "update requireplandetail set dataState ='2' where stockinfactnum>=plannum";
	$db->Execute($udpate);
	//print $udpate."<HR>";
...
function click() {
if (event.button==2) {
alert('欢迎使用通达进销存。如需技术支持与服务，请联系开发商中国兵器工业信息中心通达科技01068964532。')
}
}
...
function checkNum(numNo)
{
    ...
    if (lastNum<parseFloat(numIn)){
     if(window.confirm("本次订单数量大于应订货数量,是否继续进行？")){
     //return false;
...
<tr align="center" >
      ...
      <td nowrap  class="TableHeader" >编号</td>

      <td nowrap  class="TableHeader" >需求部门</td>

      <td nowrap  class="TableHeader" >计划日期</td>
      ...
      <td nowrap  class="TableHeader" >计划数量</td>
...
<td nowrap  class="TableHeader" >订单单价</td>
<td nowrap  class="TableHeader" >小计</td>
</tr>

<?php
$sql = "select requireplanmain.tablecode,
		department.DEPT_NAME,
		requireplanmain.tabledate,
		requireplandetail.productid,
		product.productname,
		requireplandetail.standard,
		requireplandetail.mode,
		requireplandetail.plannum,
		requireplandetail.stockinfactnum,
		requireplandetail.price,
		requireplandetail.requireplandetailsign,
		requireplandetail.requireplandetailsign,
		requireplandetail.requireplandetailsign,
		requireplandetail.requireplandetailsign,
		...
		department,
		product
		where requireplandetail.mainrowid=requireplanmain.ROWID
		and requireplanmain.stockid=department.DEPT_ID and
		requireplandetail.productid=product.productid   and
		requireplanmain.flowState='1'
		and ( requireplandetail.plannum>requireplandetail.stockinfactnum
				or requireplandetail.dataState!='2' )
...
<td   align="left" ><input readonly  type="text"   name="8<?php echo $MarkID?>" value="<?php echo $Element['stockinfactnum']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="right" ><input readonly  type="text"   name="9<?php echo $MarkID?>" value="<?php echo $Element['price']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="10<?php echo $MarkID?>" value="<?php echo $Element['requireplandetailsign']?>" size="10" maxlength="54"  class="SmallInput2"></td>
