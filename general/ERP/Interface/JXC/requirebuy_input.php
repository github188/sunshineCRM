...
if($_GET['action']=='dataDeal')		{
	require_once('dataDealRequireBuy.php');
	//print_R($db);
	if($_POST['dataDealMethod']=="update")	{
		dataDeal_edit();
		print "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=requireplanmain_newai.php'>\n";
	}
	else	{
		dataDeal_add();
		if($_GET['returnAction']=="init")	{
			print "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=requireplanmain_newai.php'>\n";
		}
		else	{
...
$requireman = $_SESSION['SUNSHINE_USER_NAME'];
$tabledate = date("Y-m-d");

if($_GET['action']=='edit_default'&&$_GET['ROWID']!="")		{
	$sql = "select * from requireplanmain where ROWID='".$_GET['ROWID']."'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();	//print_R($rs_a);

	$tablecode = $rs_a[0]['tablecode'];
	$tabledate = $rs_a[0]['tabledate'];
	$stockinpersion = $rs_a[0]['stockinpersion'];
	$stockinsign = $rs_a[0]['stockinsign'];
	$stockintype = $rs_a[0]['stockintype'];
	$userexplian = $rs_a[0]['userexplian'];
	$stockpersion = $rs_a[0]['stockpersion'];
	$stockid = $rs_a[0]['stockid'];
	$stockiplaceid = $rs_a[0]['stockiplaceid'];
	$stockstate = $rs_a[0]['stockstate'];
	$fillindate = $rs_a[0]['fillindate'];
	$exampersion = $rs_a[0]['exampersion'];
	$supplyid = $rs_a[0]['supplyid'];
	$user_id = $rs_a[0]['user_id'];
	$amt = $rs_a[0]['amt'];
	$inmess = $rs_a[0]['inmess'];
	$inmobile = $rs_a[0]['inmobile'];
	$buymess = $rs_a[0]['buymess'];
	$buymobile = $rs_a[0]['buymobile'];
	$intype = $rs_a[0]['intype'];
	$payamt = $rs_a[0]['payamt'];
	$factpayamt = $rs_a[0]['factpayamt'];
	$isfax = $rs_a[0]['isfax'];
	$requireman = $rs_a[0]['requireman'];
	$chinaAmt = $rs_a[0]['chinaAmt'];
	$fromRowId = $rs_a[0]['fromRowId'];
	$fromModuleId = $rs_a[0]['fromModuleId'];


	$sql = "select * from requireplandetail where mainrowid='".$_GET['ROWID']."'";
	$rs2 = $db->Execute($sql);
	$rs_a2 = $rs2->GetArray();	//print_R($rs_a2[0]);




	// Array ( [stockincode] => [receivecode] => [ordercode] => [productid] => LD001-02 [plannum] => 3 [stockinfactnum] => 0 [price] => 111 [money] => 333 [requireplandetailsign] => 备注 [ROWID] => 5 [state] => 4 [stockintype] => [toreturn] => [user_id] => [mainrowid] => 3 [standard] => [mode] => [dataState] => 0 [storenum] => 10.00 )
}

?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML><HEAD><TITLE>采购计划单据填写</TITLE>
<META http-equiv=Content-Type content="text/html; charset=gb2312"><LINK
href="<?php echo ROOT_DIR?>theme/3/style.css" type=text/css rel=stylesheet>
...
window.showModalDialog(fieldname,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:280px;dialogHeight:230px;dialogTop:"+mytop+"px;dialogLeft:"+myleft+"px");
}
...
function query_supply(field)
{
  ...
  table=document.all(field+"_TABLE").value;
  ModuleId=document.all("ModuleId").value;
  URL="./querycustomer/index.php?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value+"&ModuleId="+ModuleId;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:750px;dialogHeight:450px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
//alert("numdotValue="+numdotValue);
 ...
 var fieldNum2="";
  ...
  function clickInsertRow() {
    ...
    tmpStr = clientArea.innerHTML;
	//document.form1("showText").value=tmpStr;
...
<FORM name=form1 action=?action=dataDeal&returnAction=<?php echo $_GET['returnAction']?> method=post>
<table width="95%" class="TableBlock"
border=0>
  <TBODY>
  <TR>
    <TD class=TableHeader noWrap align=left colSpan=2>&nbsp;<B>采购计划单据填写 ：以下项目带*号的为必填项目</B></TD>
    <TD class=TableHeader noWrap align=left colSpan=2>&nbsp;</TD></TR>
  <TR>
    <TD class=TableContent noWrap align=right width=100><B>编号&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallInput readOnly
      maxLength=50 size=15 value=<?php echo returnDateAutoIncrement("tablecode","requireplanmain",$tablecode);?> name=tablecode> <B>*</B> </TD>
    <TD class=TableContent noWrap align=right width=100><B>计划日期&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallStatic readOnly
      value="<?php echo $tabledate?>" name=tabledate> <INPUT class=SmallButton onclick="query_date('tabledate');" type=button value=浏览 name=b1>
      <B>*</B> </TD></TR>
  <TR>
    <TD class=TableContent noWrap align=right width=100><B>需求部门&nbsp;</B></TD>
	...
	?>
	  ...
	  <B>*</B> </TD>
    <TD class=TableContent noWrap align=right width=100><B>订单金额&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallInput
      onkeypress="check_input_num('MONEY')" onblur="check_value('amt')"
      onfocus="init_value('amt')" readOnly maxLength=50 size=15 name=amt value="<?php echo $amt?>"> </TD></TR>
  <TR>
    <TD class=TableContent noWrap align=right width=100><B>计划付款&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallInput
      onkeypress="check_input_num('MONEY')" onblur="check_value('factpayamt')"
      onfocus="init_value('factpayamt')" maxLength=50 size=15 name=factpayamt value="<?php echo $factpayamt?>">
    </TD>
    <TD class=TableContent noWrap align=right width=100><B>计划类型&nbsp;</B></TD>
    <TD class=TableData noWrap align=left>
  ...
  <TR>
    <TD class=TableContent noWrap align=right width=100><B>已付定金&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallInput
      onkeypress="check_input_num('MONEY')" onblur="check_value('payamt')"
      onfocus="init_value('payamt')" maxLength=50 size=15 name=payamt value="<?php echo $payamt?>"> </TD>
    <TD class=TableContent noWrap align=right width=100><B>备注&nbsp;</B></TD>
    <TD class=TableData noWrap align=left><INPUT class=SmallInput maxLength=50
      size=15 name=stockinsign value="<?php echo $stockinsign?>"> </TD></TR>
  <TR>
    <TD class=TableContent noWrap align=right width=100><B>采购业务员&nbsp;</B></TD>
	<TD class=TableData noWrap align=left>
	  <input   type="text" name="requireman_NAME" value="<?php echo $requireman_NAME=returntablefield("user","USER_NAME",$requireman,"NICK_NAME");?>" size="15"  maxlength="50"   onclick="query_user('requireman');"  class="SmallInput">
	  <input   type="hidden" name="requireman" value="<?php echo $requireman?>" size="15"  maxlength="50">
	  </TD>
    <TD class=TableContent noWrap align=right width=100>&nbsp;</B></TD>
    <TD class=TableData noWrap align=left>&nbsp;</TD></TR><INPUT type=hidden
  value=9 name=a_count> <INPUT type=hidden
  value=,tablecode,tabledate,stockid,amt,factpayamt,intype,payamt,stockinsign,requireman
  name=Add_fieldName> <INPUT type=hidden
  value=,CHAR,DATE,CHAR,MONEY,MONEY,CHAR,MONEY,CHAR,CHAR name=Add_fieldType>
  <INPUT type=hidden value=requireplanmain name=Add_table> <INPUT type=hidden
  value=requirebuy_input.php name=Add_url> <INPUT type=hidden
  value=requirebuy_newai.php name=Back_url> <INPUT type=hidden
  value=tablecode,tabledate,stockid, name=FLD_MUST_STR> <INPUT type=hidden
  value=编号,计划日期,需求部门, name=FLD_NAME_STR> <INPUT type=hidden value=B06
  name=ModuleId> <INPUT type=hidden value=8.17 name=ExchRate> </TBODY></TABLE>
...
border=0>
  <TBODY>
  <TR>
    <TD class=TableContent noWrap align=middle width=40><B>序号</B></TD>
    <TD class=TableContent noWrap align=middle width=150><B>商品名称*</B></TD>
    <TD class=TableContent noWrap align=middle width=100><B>规格*</B></TD>
    <TD class=TableContent noWrap align=middle width=100><B>型号*</B></TD>
    <TD class=TableContent noWrap align=middle width=150><B>库存数量</B></TD>
    <TD class=TableContent noWrap align=middle width=100><B>计划数量*</B></TD>
    <TD class=TableContent noWrap align=middle width=100><B>单价</B></TD>
    <TD class=TableContent noWrap align=middle width=100><B>金额</B></TD>
    <TD class=TableContent noWrap align=middle
  width=100><B>备注</B></TD></TR></TBODY></TABLE></DIV>
<DIV id=dataId>
<?php
...
for($i=0;$i<sizeof($rs_a2);$i++)			{
	...
	$MarkName = "_".$MarkID;
	$Element = $rs_a2[$i];
	$stockincode = $Element['stockincode'];
	$receivecode = $Element['receivecode'];
	$ordercode = $Element['ordercode'];
	$productid = $Element['productid'];
	$plannum = $Element['plannum'];
	$stockinfactnum = $Element['stockinfactnum'];
	$price = $Element['price'];
	$money = $Element['money'];
	$requireplandetailsign = $Element['requireplandetailsign'];
	$ROWID = $Element['ROWID'];
	$state = $Element['state'];
...
border=0>
  <TBODY>
  <TR>
    <TD class=TableContent noWrap align=middle width=40><INPUT type=hidden
      value=<?php echo $MarkID?> name=dataNo> <B><?php echo $MarkID?></B> </TD>
    <TD class=TableData noWrap align=left width=150>
	  <input type=hidden name=Rowid_<?php echo $MarkName?> value=<?php echo $ROWID?>>
	  <INPUT
      id=id_productid_id_1_wxdid_<?php echo $MarkID?> type=hidden name=productid_id<?php echo $MarkName?>  value="<?php echo $productid?>"> <INPUT
      id=num_productid_id_1_wxdnum_<?php echo $MarkID?> type=hidden name=productid_num<?php echo $MarkName?>> <INPUT
      id=num_productid_id_1_wxdrate_<?php echo $MarkID?> type=hidden name=productid_rate<?php echo $MarkName?>> <INPUT
      class=SmallInput2 onkeypress="dataQuery('productid','B06',<?php echo $MarkID?>);"
      id=id_productid_1_wxd_<?php echo $MarkID?> ondblclick="dataQuery('productid','B06',<?php echo $MarkID?>);"
      title=请选择商品..... onclick="dataClick('<?php echo $MarkID?>'); " maxLength=53 size=15
      name=productid<?php echo $MarkName?> value="<?php echo returntablefield("product","productid",$productid,"productname");?>"> </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress=insertRow(); id=standard_<?php echo $MarkID?>  maxLength=50 size=10 name=standard<?php echo $MarkName?> value="<?php echo $standard?>">
    </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress=insertRow(); id=mode_<?php echo $MarkID?> maxLength=50 size=10 name=mode<?php echo $MarkName?> value="<?php echo $mode?>"> </TD>
    <TD class=TableData noWrap align=left width=150><INPUT class=SmallInput2
      onkeypress=insertRow(); id=storenum_<?php echo $MarkID?> maxLength=50 size=15 name=storenum<?php echo $MarkName?> value="<?php echo $storenum?>">
    </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress="check_input_num('NUMBER')" id=plannum_<?php echo $MarkID?>
      onblur="check_value_order('price_<?php echo $MarkID?>')"
      onfocus="init_value_order('price_<?php echo $MarkID?>')" maxLength=54 size=10 name=plannum<?php echo $MarkName?> value="<?php echo $plannum?>">
    </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress="check_input_num('MONEY')" id=price_<?php echo $MarkID?>
      onblur="check_value_order('price_<?php echo $MarkID?>')"
      onfocus="init_value_order('price_<?php echo $MarkID?>')" maxLength=56 size=10 name=price<?php echo $MarkName?> value="<?php echo $price?>"> </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress="check_input_num('MONEY')" id=money_<?php echo $MarkID?>
      onblur="check_value_order('money_<?php echo $MarkID?>')"
      onfocus="init_value_order('money_<?php echo $MarkID?>')" readOnly maxLength=57 size=10
      name=money<?php echo $MarkName?> value="<?php echo $money?>"> </TD>
    <TD class=TableData noWrap align=left width=100><INPUT class=SmallInput2
      onkeypress=insertRow(); id=requireplandetailsign_<?php echo $MarkID?> maxLength=58 size=10
      name=requireplandetailsign<?php echo $MarkName?> value="<?php echo $requireplandetailsign?>"> </TD></TR></TBODY></TABLE>
<?php
}//end for
?>
</DIV>
...
<!--
<textarea name=showText rows=5 cols=105></textarea>
-->
<DIV id=hiddenId>
<INPUT type=hidden name=flag>
<INPUT type=hidden name=actionStateId>
<INPUT type=hidden value=8 name=s_count>
<INPUT type=hidden value=<?php echo $dataCount?> name=dataCount>
<INPUT type=hidden value=4 name=StockOutType>
<INPUT type=hidden value=,productid,standard,mode,storenum,plannum,price,money,requireplandetailsign name=secAdd_fieldName>
<INPUT type=hidden value=,CHAR,CHAR,CHAR,CHAR,NUMBER,MONEY,MONEY,CHAR name=secAdd_fieldType>
<INPUT type=hidden value=requireplandetail name=secAdd_table>
<INPUT type=hidden value=productid,standard,mode,plannum, name=secFLD_MUST_STR>
<INPUT type=hidden value=商品名称,规格,型号,计划数量, name=secFLD_NAME_STR>
...
<INPUT class=SmallButton id=btninsert onclick=javascript:clickInsertRow(); type=button value=添加明细 name=btninsert>&nbsp;&nbsp;&nbsp;
<INPUT class=SmallButton id=btnsave onclick="javascript:clickDeleteRow() ;" type=button value=删除明细 name=btnsave>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<INPUT class=SmallButton id=btnsave onclick="javascript:secorderaddnew() ;" type=button value=保存 name=btnsave>&nbsp;&nbsp;&nbsp;
<INPUT class=SmallButton id=btnlist type=reset value=重置 name=btnlist>&nbsp;&nbsp;&nbsp;
<INPUT class=SmallButton id=close onclick="location='requireplanmain_newai.php'" type=button value=返回 name=close>
<BR>本页面执行时间:
<SCRIPT>
 var now1 =new Date()
 EndTime_S=now1.getTime()
 document.write ((EndTime_S-StarTime_S)/1000+"秒");
...
<DIV id=hiddenDataId>
<table width="95%" class="TableBlock" style="DISPLAY: none"
bgColor=#000000 border=0>
  <TBODY>
  <TR>
    <TD class=TableContent noWrap align=middle width=40>
	<INPUT type=hidden  value=1 name=dataNo> <B>1</B></TD>
    <TD class=TableData noWrap align=left width=150>
	  <INPUT id=id_productid_id_1_wxdid_1 type=hidden name=productid_id_dxw>
	  <INPUT id=num_productid_id_1_wxdnum_1 type=hidden name=productid_num_dxw>
	  <INPUT id=num_productid_id_1_wxdrate_1 type=hidden name=productid_rate_dxw>
	  <INPUT class=SmallInput2 onkeypress="dataQuery('productid','B06',1);"
      id=id_productid_1_wxd_1 ondblclick="dataQuery('productid','B06',1);"
      title=请选择商品..... onclick="dataClick('1'); " maxLength=53 size=15
      name=productid_dxw>
	  </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2 onkeypress=insertRow(); id=standard_dxw maxLength=50 size=10       name=standard_dxw> </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2
      onkeypress=insertRow(); id=mode_dxw maxLength=50 size=10 name=mode_dxw> </TD>
    <TD class=TableData noWrap align=left width=150>
	<INPUT class=SmallInput2
      onkeypress=insertRow(); id=storenum_dxw maxLength=50 size=15
      name=storenum_dxw> </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2
      onkeypress="check_input_num('NUMBER')" id=plannum_dxw
      onblur="check_value_order('price_dxw')"
      onfocus="init_value_order('price_dxw')" maxLength=54 size=10 name=plannum_dxw>
    </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2
      onkeypress="check_input_num('MONEY')" id=price_dxw
      onblur="check_value_order('price_dxw')"
      onfocus="init_value_order('price_dxw')" maxLength=56 size=10 name=price_dxw>
    </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2
      onkeypress="check_input_num('MONEY')" id=money_dxw
      onblur="check_value_order('money_dxw')"
      onfocus="init_value_order('money_dxw')" readOnly maxLength=57 size=10
      name=money_dxw> </TD>
    <TD class=TableData noWrap align=left width=100>
	<INPUT class=SmallInput2
      onkeypress=insertRow(); id=requireplandetailsign_dxw maxLength=58 size=10
      name=requireplandetailsign_dxw> </TD></TR></TBODY></TABLE></DIV></BODY></HTML>
