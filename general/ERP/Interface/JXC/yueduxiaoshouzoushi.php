...
//}
//print_r($list);exit;
$sql="SELECT ".$date_format." as name,sum(a.totalmoney - a.oddment) as sum FROM sellplanmain a WHERE a.user_flag>0 and a.createtime>='".$start_time."' and a.createtime<'".date("Y-m-1",strtotime("+1months",strtotime($end_time)))."' GROUP BY name";
//exit($sql);
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();
foreach ($rs_a as $row){
...
chart_index.setDataXML("<graph bgcolor='e1f5ff' caption='月度销售走势图' subCaption='精确到百位（四舍五入）' numberPrefix='' formatNumberScale='1' decimalPrecision='2' baseFontSize='14' numberSuffix='万'><?php foreach ($list as $key=>$vo){echo "<set name='".$key."' value='".($vo/10000)."'/>";}?></graph>");
