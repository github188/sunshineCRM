...
if($_GET['action']=="dataDeal")			{
	//print_R($_POST);
	//print_R($_SESSION);
	$tablecode = returnDateAutoIncrement("tablecode","requireplanmain");
	...
	$countAllAmt_array = explode(',',$_POST['countAllAmt']);
	$countAllAmt = join('',$countAllAmt_array);
	$countAllAmt = (int)$countAllAmt;
	$chinaAmt = $_POST['chinaAmt'];
	$state = '1';					$_POST['state'];
	$flowState = '-1';
	$intype = $_POST['intype'];
	$sendDate = date("Y-m-d");
	$stockinsign = "来自于销售订单单";

	$sql = "insert into requireplanmain (
		tablecode,tabledate,stockid,stockiplaceid,
		supplyid,amt,factpayamt,ROWID,user_id,
		state,intype,chinaAmt,requireman,fillindate,
		payamt,stockinsign,flowState
		)
		values (
		'$tablecode','$tabledate','$stockid','',
		'$supplyid','$countAllAmt','$countAllAmt','','$USER_NAME',
		'$state','$intype','$chinaAmt','$USER_NAME','$sendDate',
	...
	for($i=1;$i<=$_POST['num'];$i++)		{
		...
		$backamt = filterAmt($_POST["backamt".$i]);
		$backallamt = filterAmt($_POST["backallamt".$i]);
		$stocknum = $_POST["stocknum".$i];
		$tablecode = $_POST["0".$i];
		$DEPT_NAME = $_POST["1".$i];
		$tabledate = $_POST["2".$i];
		$productid = $_POST["3".$i];
		...
		$price = $_POST["9".$i];
		$sellplandetailsign = $_POST["10".$i];
		$ROWID = $_POST["ROWID_".$i];


		//$ordercode = $_POST['ordercode'];

		//当时流转数据不为0时，转换采购订单明细到采购订单。
		if($backnum!="")		{
			$sql = "insert into requireplandetail (
			productid,plannum,stockinfactnum,price,money,
			mainrowid,standard,mode,ordercode
			) values (
			'$productid','$backnum','$stockinfactnum', '$price','$money',
			'$Insert_ID','$standard','$mode','$ordercode')";
			$db->Execute($sql);
			//print $sql."<HR>";

			//修改采购订单明细的数据状态为部分执行
			$update = "update sellplandetail set dataState ='1',stockinfactnum=stockinfactnum+'$backnum',buynum=buynum+'$backnum' where ROWID='$ROWID'";
			$db->Execute($update);
			//print $update."<HR>";

			//修改采购订单主表的数据状态为部分执行
			$MAINROWID = returntablefield("sellplandetail","ROWID",$ROWID,"mainrowid");
			$update = "update sellplanmain set dataState ='1' where ROWID='$MAINROWID'";
			$db->Execute($update);
			//print $update."<HR>";

			//判断主表里面的数据是否完全执行
			$udpate = "select DISTINCT mainrowid from sellplandetail where (dataState!='2' and stockinfactnum>=plannum) and mainrowid='$MAINROWID'";
...
function order()
{
  ...
  URL="/order.jsp?ModuleId="+ModuleId;
  loc_x=150;
  loc_y=100;
...
function selectMan(ModuleId)
{
  ...
  URL="/inflow/index/orderprod.jsp?ModuleId="+idStr;
  loc_x=150;
  loc_y=100;
  //window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:700px;dialogHeight:450px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"11","edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:350px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
function selectField(ModuleId)
{
  ...
  idStr=idStr.substring(0,idStr.indexOf(","));
  URL="/inflow/index/orderfield.jsp?FolderID="+idStr+"&ModuleId="+ModuleId;
  loc_x=150;
  loc_y=100;
  //window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:700px;dialogHeight:450px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"11","edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:350px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
{
  URL="/module/combquery/index.jsp?ModuleId="+ModuleId;
  loc_x=400;
  loc_y=150;
...
{
  URL="/module/helpquery/index.jsp?ModuleId="+ModuleId+"&ModuleActionId="+ModuleActionId;
  loc_x=400;
  loc_y=150;
  //window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:150px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"11","edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:350px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
{
  URL="/export.jsp?ModuleId="+ModuleId+"&ModuleName="+ModuleName;
  loc_x=450;
  loc_y=250;
...
{
  URL="/query.jsp?ModuleId="+ModuleId+"&ModuleName="+ModuleName;
  loc_x=450;
  loc_y=250;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:150px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"qqq","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=300px,height=250px,Top="+loc_y+"px,Left="+loc_x+"px");
      ...
      else
      {
        //alert("不相等");
         if(idStr!=""&&document.all("TR"+idStr.replace(",",""))!=null)
         {
            ...
            //document.all("TR"+idStr.replace(",","")).style.color="#ffffff";
         }
         idStr+=theId;
...
function query_code(field)
{
  ...
  URL="/query/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
...
function query_user(field)
{
  ...
  code="1";
  table="TD_OA.user";
  URL="/queryuser/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:450px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
function query_userid(field)
{
  ...
  code="1";
  table="TD_OA.user";
  URL="/queryuserid/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:450px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
function query_bath(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/bathquery/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
...
function query_two(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/lever/code/query/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:300px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
function query_acct(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/queryacct/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:300px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
function query_ongrpno(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/ongrpnoquery/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  //URL="/ongrpnoquery/index.jsp?FIELD="+field;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
...
function query_ongrpno_exps(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/ongrpnoqueryexps/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  //URL="/ongrpnoquery/index.jsp?FIELD="+field;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:350px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
function query_pay(field)
{
  ...
  code=document.all(field+"_CODE").value;
  table=document.all(field+"_TABLE").value;
  URL="/payquery/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:300px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
{
  init_value=field;
  URL="/inc/calendar.jsp?FIELDNAME="+init_value;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
...
{
  init_value=document.all(field).value;
  URL="/voucher/index.jsp?FIELDNAME="+field+"&voucher="+ModuleId+"&INIT_VALUE="+init_value+"&row="+row+"&typeid="+typeid;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
...
function balance_query_code(field,grp_no)
{
  ...
  URL="/balancequery/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value+"&GRP_NO="+grp_no;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:300px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
{
   URL="/detail.jsp?rowid="+idRowid+"&ModuleId="+ModuleId;
   loc_x=254;
   loc_y=162;
...
function datadetailclick(idRowid,ModuleId)
{
   ...
   URL="/datadetail.jsp?rowid="+idRowid+"&ModuleId="+ModuleId+"&stockName="+stockName+"&fromdata="+fromdata+"&enddata="+enddata;
   loc_x=254;
   loc_y=162;
  window.open(URL,"qq","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=650px,height=250px,Top="+loc_y+"px,Left="+loc_x+"px");
}
...
function datadetail(State)
{
   ...
   var stockName=document.Qform1("stockName").value;
	 var fromdata=document.Qform1("fromdata").value;
	 var enddata=document.Qform1("enddata").value;
   URL="/datadetail.jsp?rowid="+idRowid+"&ModuleId="+ModuleId+"&stockName="+stockName+"&fromdata="+fromdata+"&enddata="+enddata+"&State="+State;
   loc_x=100;
   loc_y=100;
...
function datadetailchart(chartState)
{
   ...
   URL="/setSql6.jsp?rowid="+idRowid+"&ModuleId="+ModuleId+"&groupByStr="+groupByStr+"&fromdata="+fromdata+"&enddata="+enddata+"&flowState="+flowState+"&dataState="+dataState+"&chartState="+chartState;//
   loc_x=254;
   loc_y=162;
   //alert(URL);
   window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:350px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
function dataPrint()
{
   ...
   URL="/setSqlPrint.jsp?rowid="+idRowid+"&ModuleId="+ModuleId;//
   loc_x=254;
   loc_y=162;
   //alert(URL);
   //window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:400px;dialogHeight:350px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
...
function datachart()
{
   ...
   var stockName=document.Qform1("stockName").value;
	 var fromdata=document.Qform1("fromdata").value;
	 var enddata=document.Qform1("enddata").value;
   URL="/datachart.jsp?rowid="+idRowid+"&ModuleId="+ModuleId+"&stockName="+stockName+"&fromdata="+fromdata+"&enddata="+enddata;
   loc_x=254;
   loc_y=162;
...
function detailsec(ModuleId)
{
   ...
   idRowid=idRowid.substring(0,idRowid.length-1);
   URL="/detailsec.jsp?rowid="+idRowid+"&ModuleId="+ModuleId;
   loc_x=10;
   loc_y=10;
   //alert(URL);
...
function flow()
{
   ...
   var ModuleId=document.all("ModuleId").value;
   URL="/flowdetail.jsp?rowid="+idRowid+"&ModuleId="+ModuleId;
   loc_x=254;
   loc_y=162;
  window.open(URL,"qq","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=650px,height=250px,Top="+loc_y+"px,Left="+loc_x+"px");
}
function backStock()
{
   ...
   var ModuleId=document.all("ModuleId").value;
   URL="/back.jsp?rowid="+idRowid+"&ModuleId="+ModuleId;
   loc_x=254;
   loc_y=162;
 	...
 	function detailFlow(dataState)
{
   ...
   var ModuleId=document.all("ModuleId").value;
   URL="/detailflow.jsp?rowid="+idRowid+"&ModuleId="+ModuleId+"&dataState="+dataState;
   loc_x=100;
   loc_y=100;
...
function query_stock(field)
{
  ...
  URL="/querystock/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value+"&ModuleId="+ModuleId;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
	...
	function editSate(StateId)
{
   ...
   URL="/changestate/index.jsp?rowid="+idStr+"&ModuleId="+ModuleId+"&StateId="+StateId;
...
function query_supply(field)
{
  ...
  table=document.all(field+"_TABLE").value;
  ModuleId=document.all("ModuleId").value;
  URL="/querycustomer/index.jsp?CODE="+code+"&TABLE="+table+"&FIELD="+field+"&INIT_VALUE="+init_value+"&ModuleId="+ModuleId;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:750px;dialogHeight:450px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
function check_value2(field)
{
  ...
  URL="/checkvalue/index.jsp?ModuleId="+ModuleId+"&Add_table="+Add_table+"&INIT_VALUE="+init_value+"&field="+field;
  loc_x=document.body.scrollLeft+event.clientX-event.offsetX+350;
  loc_y=document.body.scrollTop+event.clientY-event.offsetY+100;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:450px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  //window.open(URL,"","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=400px,height=300px,Top="+loc_y+"px,Left="+loc_x+"px");
...
//alert("numdotValue="+numdotValue);
 ...
 var fieldNum2="";
  ...
  URL="/dataquery/index.jsp?moduleId="+moduleId+"&fieldname="+fieldname+"&fieldValue="+fieldValue+"&codeValue="+codeValue+"&countStr="+countStr+"&cliNo="+cliNo+"&supplyId="+supplyId+"&stockId="+stockId+"&typeId="+typeId;
   loc_x=0;
   loc_y=0;
   //alert(URL);
  ...
  URL="/orderquery/index.jsp?moduleId="+moduleId+"&fieldname="+fieldname+"&fieldValue="+fieldValue+"&codeValue="+codeValue+"&countStr="+countStr+"&cliNo="+cliNo+"&supplyId="+supplyId+"&tableDate="+tableDate;
   loc_x=354;
   loc_y=262;
  window.showModalDialog(URL,self,"edge:raised;scroll:0;status:0;help:0;resizable:1;dialogWidth:350px;dialogHeight:250px;dialogTop:"+loc_y+"px;dialogLeft:"+loc_x+"px");
  }
  ...
  var fieldValue=document.form1("id_"+fieldname+"_"+countStr+"_wxd_"+cliNo).value;
  var codeValue=document.form1("id_"+fieldname+"_id"+"_"+countStr+"_wxdid_"+cliNo).value;
  URL="/amtquery/index.jsp?moduleId="+moduleId+"&fieldname="+fieldname+"&fieldValue="+fieldValue+"&codeValue="+codeValue+"&countStr="+countStr+"&cliNo="+cliNo+"&supplyId="+supplyId;
   loc_x=354;
   loc_y=262;
...
function doIn()
{
   ...
   idRowid=idRowid.substring(0,idRowid.length-1);
   var ModuleId=document.all("ModuleId").value;
   URL="/doStock.jsp?mainrowid="+idRowid+"&ModuleId="+ModuleId+"&doState='0'";
   loc_x=254;
   loc_y=162;
  window.open(URL,"qq","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=650px,height=250px,Top="+loc_y+"px,Left="+loc_x+"px");
}
function doOut()
{
   ...
   idRowid=idRowid.substring(0,idRowid.length-1);
   var ModuleId=document.all("ModuleId").value;
   URL="/outStock.jsp?mainrowid="+idRowid+"&ModuleId="+ModuleId+"&doState='1'";
   loc_x=254;
   loc_y=162;
  window.open(URL,"qq","menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1,width=650px,height=250px,Top="+loc_y+"px,Left="+loc_x+"px");
}
 ...
 function stockBack(num)
{
    ...
    countNum=countNum+parseFloat(backNum);//数量的累加
...
function checkNum(numNo)
{
    ...
    var num1=document.form1("7"+numNo).value;//计划数量
    var numCount=document.form1("num").value;//总行数
    var num2=document.form1("8"+numNo).value; //到货数量
    var numIn=document.form1("backnum"+numNo).value;
    var stockNum=document.form1("stockNum"+numNo).value;

    if (document.form1("backamt"+numNo).value==null||document.form1("backamt"+numNo).value==""){
    document.form1("backamt"+numNo).value=document.form1("9"+numNo).value; //本次到货数量
    }
    ...
    if (lastNum<parseFloat(numIn)){
     if(window.confirm("本次"+'采购'+"数量大于应"+'采购'+"数量,是否继续进行？"))
     {
   ...
   if  (parseFloat(stockNum)<parseFloat(numIn)&&DMState=="DP"&&jsModuleId=="B04"&&stockZero=="1"){//
     if(window.confirm("本次"+'采购'+"数量大于实际库存数量,是否继续进行？"))
    {
      document.form1("backallamt"+numNo).value="";
      document.form1("backamt"+numNo).value="";
      document.form1("backnum"+numNo).value="";
...
function  autoCom(){
    ...
    for (var i=1;i<=numCount;i++){
    	var num1=document.form1("7"+i).value;//计划数量
    	var num2=document.form1("8"+i).value; //到货数量
      var numIn=document.form1("backnum"+i).value;
      var stockNum=document.form1("stockNum"+i).value;
      var priceNum=document.form1("9"+i).value; //单价
      if (numIn==null||numIn==""){
      	var buyNum=parseFloat(num1)-parseFloat(num2)-parseFloat(stockNum);
      	if (buyNum>0){
      		document.form1("backnum"+i).value=buyNum;
      		document.form1("backamt"+i).value=priceNum;
      		document.form1("backallamt"+i).value=buyNum*parseFloat(priceNum);
      		}
      }else{
      	  document.form1("backnum"+i).value="";
      		document.form1("backamt"+i).value="";
      		document.form1("backallamt"+i).value="";
      	}
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Small1" align="right"><b><font color="#000000">
    	<input   type=radio  name="isAddNum"  checked  value=2   class="SmallInput">数量合计
		&nbsp&nbsp&nbsp&nbsp
		<input   type=radio  name="isAddNum"    value=1   class="SmallInput">数量分开</font></b>
    </td>
  </tr>
</table>
...
<tr align="center" >
      ...
      <td nowrap  class="TableHeader" >订单编号</td>

      <td nowrap  class="TableHeader" >销售部门</td>

      <td nowrap  class="TableHeader" >订单日期</td>

      <td nowrap  class="TableHeader" >商品编码</td>

      <td nowrap  class="TableHeader" >商品名称</td>

      <td nowrap  class="TableHeader" >规格</td>

      <td nowrap  class="TableHeader" >型号</td>

      <td nowrap  class="TableHeader" >订单数量</td>

      <td nowrap  class="TableHeader" >已定货数量</td>
...
<td nowrap  class="TableHeader" >库存数量</td>
<td nowrap  class="TableHeader" >采购数量</td>
<td nowrap  class="TableHeader" >采购单价</td>
<td nowrap  class="TableHeader" >小计</td>
</tr>
...
$sql = "select
			...
			sellplanmain.tabledate,
			product.productid,
			product.productname,
			sellplandetail.standard,
			sellplandetail.mode,
			sellplandetail.plannum,
			sellplandetail.buynum,
			sellplandetail.price,
			sellplandetail.price*sellplandetail.plannum as money,
			sellplandetail.sellplandetailsign,
			sellplandetail.ROWID,
			store.stocknum
		from sellplandetail,
			sellplanmain,department,
			product,
			store
		where
			...
			sellplanmain.flowState='1'    and
			(
			sellplandetail.plannum>sellplandetail.buynum or
			sellplandetail.dataState!='2'
			)
			and
			product.productid = store.productid
		order by tablecode
		";
	//where sellplandetail.plannum>sellplandetail.buynum or
	//where and  ( product.producttype='77' or product.producttype='80'  )
//print $sql;
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
...
<td   align="left" ><input readonly  type="text"   name="0<?php echo $MarkID?>" value="<?php echo $Element['tablecode']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="1<?php echo $MarkID?>" value="<?php echo $Element['DEPT_NAME']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="2<?php echo $MarkID?>" value="<?php echo $Element['tabledate']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="3<?php echo $MarkID?>" value="<?php echo $Element['productid']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="4<?php echo $MarkID?>" value="<?php echo $Element['productname']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="5<?php echo $MarkID?>" value="<?php echo $Element['standard']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="6<?php echo $MarkID?>" value="<?php echo $Element['mode']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="7<?php echo $MarkID?>" value="<?php echo $Element['plannum']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="8<?php echo $MarkID?>" value="<?php echo $Element['buynum']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="9<?php echo $MarkID?>" value="<?php echo $Element['price']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="10<?php echo $MarkID?>" value="<?php echo $Element['money']?>" size="10" maxlength="54"  class="SmallInput2"></td>

<td   align="left" ><input readonly  type="text"   name="11<?php echo $MarkID?>" value="<?php echo $Element['sellplandetailsign']?>" size="10" maxlength="54"  class="SmallInput2"></td>


<td  ><input   type="text"   name="stocknum<?php echo $MarkID?>" size="10" maxlength="54"  readonly value="<?php echo $Element['stocknum']?>"  class="SmallInput2"></td>
<td  ><input   type="text"   name="backnum<?php echo $MarkID?>" size="10" maxlength="54"  onblur="checkNum('<?php echo $MarkID?>')"  class="SmallInput2"></td>
<td  ><input   type="text"   name="backamt<?php echo $MarkID?>" size="10" maxlength="54"  onblur="checkNum('<?php echo $MarkID?>')"   class="SmallInput2"></td>
<td  ><input   type="text"   name="backallamt<?php echo $MarkID?>" size="10" maxlength="54"   class="SmallInput2"></td>
...
<td class="TableHeader" align="left"><?php echo $All_price?>&nbsp</td>
<td class="TableHeader" align="left">&nbsp</td>
<td class="TableHeader" align="left">&nbsp</td>

<td class="TableHeader" align="left" >
	<input   type="text"   name="countNum" size="10" maxlength="54"   class="SmallInput2">
	</td>
<td  class="TableHeader" align="left">
	<input   type="text"   name="countAmt" size="10" maxlength="54"   class="SmallInput2">
	</td>
<td  class="TableHeader" align="left">
	<input   type="text"   name="countAllAmt" size="10" maxlength="54"   class="SmallInput2">
	</td>
</tr>
</table>
...
<input type="button" name="close" id="close" class="SmallButton" value="关闭" onClick="javascript:parent.window.close();">
</body>
</html>
