...
if($_GET['name']=="dalou"){
			...
			$row_sql = "select COUNT(*) AS sum from wu_costsummary where 费用年月份='".$缴费年月份."' and 本次实收='0.00'";
			...
			if($total1>0){		
				...
				$sql = "select * from wu_costsummary where 费用年月份='".$缴费年月份."' and 本次实收='0.00' order by 本次应收 desc LIMIT $offset,$each_page";
...
if($queren = "true"){
		foreach((array)$单元 as $val){
					$rows_sql = "select COUNT(*) AS sum from wu_costsummary where 单元编号='$val' and 费用年月份='$缴费年月份' and 本次实收='0.00'";
					$rows_rs = $db -> Execute($rows_sql);
					$total = $rows_rs -> fields['sum'];

					if($total>0){
						
						$sql = "select * from wu_costsummary where 单元编号='$val' and 费用年月份='$缴费年月份' and 本次实收='0.00'order by 单元编号 desc";
						$rs = $db -> Execute($sql);
						$rs_a = $rs -> GetArray();
...
-->
    <tr align="center" class="tableContent">
	     <td colspan="10" >
		 <INPUT TYPE="button" VALUE="导出" class="SmallButton"  align="left" ONCLICK="location='?pageAction=ExportDataToFile&单元编号=<?php echo $_GET['业主房号']?>&费用年月份=<?php echo $_GET['费用年月份']?>'">
		 <?php 
    ...
    <tr align="center" class="tableContent">
		...
		<font color="green">说明：</font><br>
			&nbsp;&nbsp;&nbsp;&nbsp;1 默认数据时本月欠费情况列表，<font color="red">数据是 <?php if($_GET['name']=="dalou"){
				echo $缴费年月份;
		    }else{
		        $时间 = date("Y-m",mktime(1,1,1,date("m")-1,date("d"),date("Y")));
		        echo $时间;
		    }?> 月数据</font>.<br>
			&nbsp;&nbsp;&nbsp;&nbsp;2 查询出来本次实收项目数据为“<font color="red">红</font>”色时，说明此项欠费，如果缴费点击缴费按钮进行缴费,将本次实收进行修改然后保存,如果用户需要缴费清单，请点击缴费按钮进入,然后打印.<br>
