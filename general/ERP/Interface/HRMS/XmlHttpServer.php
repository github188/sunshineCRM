...
if($_GET['action']=="showdatas"&&$_GET['selectName']!="")
{
	...
	if($_GET['CurXueQi']!="")
		...
		$sql = "select 开课教师,教师用户名 from edu_planexec where 班级名称='".$班级名称."' and 开课学期='$开课学期' and 课程名称='$课程名称' order by 教师用户名,班级代码";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
	...
	$sql = "select distinct 开课教师,教师用户名 from edu_planexec where 班级名称='".$_GET['班级名称']."' and 级别 = '$NJ' and 开课学期='$开课学期' and 课程名称='".$_GET['selectName']."'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	//print_R($rs_a);
	for($ii=0;$ii<sizeof($rs_a);$ii++)		{
...
if($_GET['action']=="Schedule"&&$_GET['selectName']!="")
{
	...
	//用于生成单双周时判断的语句,如果存在条件,那么显示冲突,中止插入操作
	if($单双周=="")			{
		$DanShuangZhouSQL = " and (单双周='0' or 单双周='1' or 单双周='2')";
		$DanShuangInsert = "0";
	}
	if($单双周=="1")			{
		$DanShuangZhouSQL = " and (单双周='0' or 单双周='1')";
		$DanShuangInsert = "1";
	}
	if($单双周=="2")			{
		$DanShuangZhouSQL = " and (单双周='0' or 单双周='2')";
		$DanShuangInsert = "2";
	}
	...
	if($_GET['checkAction']=="doit")		{
		...
		$rs = $db->Execute($sql);
		if($rs->fields['NUM']>0)		{
			print $returnText .= "<font color=red><B>* 该教室在该时间已经安按有课程！<BR></B></font><BR>";
			//重新获取数据
			$returnCourseStatistics = '';
			$returnCourseStatistics = returnCourseStatistics($CurXueQi,$ClassCode);
			$returnCourseStatistics!=""?$returnCourseStatistics="::".$returnCourseStatistics:'';
		...
		*/
		...
		$sql = "select count(教师) as NUM from edu_schedule where `学期`='$CurXueQi' and `班级`='$ClassCode' and `星期`='$Week' and `节次`='$JieCi' $DanShuangZhouSQL";
		$rs = $db->Execute($sql);
		if($rs->fields['NUM']>0)		{
			print $returnText .= " <font color=red><B>* 该教师在该时间已经安按有课程！</B></font><BR>";
			//重新获取数据
			$returnCourseStatistics = '';
			$returnCourseStatistics = returnCourseStatistics($CurXueQi,$ClassCode);
			$returnCourseStatistics!=""?$returnCourseStatistics="::".$returnCourseStatistics:'';
		...
		//判断教师是否冲突 教室－学期－星期－节次 即同一教师不能在同一时间出现在两个教室
		$sql = "select count(教师) as NUM from edu_schedule where `学期`='$CurXueQi' and `星期`='$Week' and `节次`='$JieCi' and `教师`='$TeacherName' and `教师用户名`='$TeacherCode' and `教室`!='$fixedClassroom' $DanShuangZhouSQL";
		$rs = $db->Execute($sql);
		if($rs->fields['NUM']>0)		{
			print $returnText .= " <font color=red><B>* 该教师不能在同一时间出现在两个教室！<BR></B></font><BR>";
			//重新获取数据
			$returnCourseStatistics = '';
			$returnCourseStatistics = returnCourseStatistics($CurXueQi,$ClassCode);
			$returnCourseStatistics!=""?$returnCourseStatistics="::".$returnCourseStatistics:'';
		...
		$sql = "select `课程`,班级,教室 from edu_schedule where `学期`='$CurXueQi' and `教室`='$fixedClassroom' and `教师`='$TeacherName' and `教师用户名`='$TeacherCode' and `星期`='$Week' and `节次`='$JieCi' $DanShuangZhouSQL";
		$rs = $db->Execute($sql);
		//print $sql.";<BR>";
		$rs_a = $rs->GetArray();
		//print_R($rs_a);
	...
	if($_GET['checkAction']=="nodo")		{
		...
		//判断是否该班在该时间段安排有课
		if($单双周==''||$单双周==0)					{
			$sql = "delete from edu_schedule where `学期`='$CurXueQi' and `班级`='$ClassCode' and `星期`='$Week' and `节次`='$JieCi' and 单双周='0'";
			$rs = $db->Execute($sql);
			...
			//判断单双周是否为0
			$sql = "select 单双周 from edu_schedule where `学期`='$CurXueQi' and `班级`='$ClassCode' and `星期`='$Week' and `节次`='$JieCi'";
			$rs = $db->Execute($sql);
			$单双周_INNER = $rs->fields['单双周'];
			if($单双周_INNER=='0')		{
				if($单双周=="1")	$单双周_NEW = '2';
				else				$单双周_NEW = '1';
				$sql = "update edu_schedule set 单双周 = '$单双周_NEW' where `学期`='$CurXueQi' and `班级`='$ClassCode' and `星期`='$Week' and `节次`='$JieCi'";
			}
			else	{
				$sql = "delete from edu_schedule where `学期`='$CurXueQi' and `班级`='$ClassCode' and `星期`='$Week' and `节次`='$JieCi' and 单双周='$单双周'";
			}
			$rs = $db->Execute($sql);
			$returnText = "<font color=green><B>* 该位置课程信息取消成功！</B></font><BR>";
			//重新获取数据
