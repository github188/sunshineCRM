...
$SortArray = array();
//处理基本表数据
$sql = "select 资产编号,资产名称,SUM(数量) AS 资产数量,SUM(数量*单价) AS 资产总金额,所属部门,资产名称,资产编号 from fixedasset where 购买日期>='$开始时间' and 购买日期<='$结束时间' and 资产名称!='' and 所属状态!='资产已报废' group by 所属部门,资产名称";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
...
<td colspan=4>从 ".$_GET['开始时间']." 到 ".$_GET['结束时间']." 数量合计:".$总数."个 金额合计:".$资产总金额."元</td>
";
for($xi=0;$xi<sizeof($部门列表);$xi++)		{
	$部门名称 = $部门列表[$xi];
	$Index = $所属部门列表[$部门名称];
	print "<td>&nbsp;数量:".$资产信息汇总DEPT['资产数量'][$部门名称]." 金额:".$资产信息汇总DEPT['资产总金额'][$部门名称]."</td>";
	$现有库存ALL += $Index;
}
print "</tr>";
print "</table>";
...
for($IX=0;$IX<sizeof($所属状态列表Array);$IX++)		{
	...
	<td>&nbsp;".($IX+1)."</td>
	<td>&nbsp;$所属状态</td>
	<td>&nbsp;<a href=\"fixedasset_view.php?".base64_encode("所属状态=$所属状态")."\" target=_blank>$资产数量</a></td>
	<td>&nbsp;$资产金额</td>
	";
}
$现有库存ALL = (int)$现有库存ALL;
...
print "<tr class=TableContent>
<td colspan=4>从 ".$_GET['开始时间']." 到 ".$_GET['结束时间']." 数量合计:".$总数."个 金额合计:".$资产总金额."元 $资产总金额大写</td>
";
print "</tr>";
print "</table></div>";
