...
function Asc_Teacher_KaoQin_ToDay_Data($考勤日期,$教师姓名,$教师用户名)			{
	...
	$sql = "select * from td_edu.edu_teacherkaoqin where 考勤日期='$考勤日期' and 教师用户名='$教师用户名'";
	...
	global $SHOWTEXT; if($SHOWTEXT)print "<BR>同步教师当天考勤数据====================================================";
...
function DealTimeJieCiShangKe($教师姓名,$教师用户名,$考勤日期,$时间)		{
	...
	$sql = "select * from td_edu.edu_teacherkaoqinmingxi where 教师用户名='$教师用户名' and 考勤日期='$考勤日期' and 上课刷卡BGN<='$迟到时间' and 上课刷卡END>='$迟到时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	//print_R($rs2_a);
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		$编号 = $rs2_a[$iii]['编号'];
		$sql = "update td_edu.edu_teacherkaoqinmingxi set 上课实际刷卡 = '$时间' ,上课考勤状态='上课迟到' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
	...
	//定义正常打卡
	$sql = "select * from td_edu.edu_teacherkaoqinmingxi where 教师用户名='$教师用户名' and 考勤日期='$考勤日期' and 上课刷卡BGN<='$时间' and 上课刷卡END>='$时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	//print_R($rs2_a);
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		$编号 = $rs2_a[$iii]['编号'];
		$sql = "update td_edu.edu_teacherkaoqinmingxi set 上课实际刷卡 = '$时间' ,上课考勤状态='正常刷卡' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
	...
	//###########################################################################
	//同步教师听课考勤数据开始（上课）
	$sql = "select * from td_edu.edu_tingke where 听课教师姓名='$教师姓名' and 听课日期='$考勤日期' and 上课刷卡BGN<='$迟到时间' and 上课刷卡END>='$迟到时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	//print_R($rs2_a);
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		...
		$sql = "update td_edu.edu_tingke set 上课打卡时间 = '$时间' ,上课考勤状态='上课迟到',最迟填写时间 = '$lll' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
		//$returnText .= "$教师姓名 上课迟到 $考勤日期 $时间<BR>";
		global $SHOWTEXT; if($SHOWTEXT)print $sql."<BR>";
	}
	$sql = "select * from td_edu.edu_tingke where 听课教师姓名='$教师姓名' and 听课日期='$考勤日期' and 上课刷卡BGN<='$时间' and 上课刷卡END>='$时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	//print_R($rs2_a);
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		$编号 = $rs2_a[$iii]['编号'];
		$lll=date("Y-m-d",strtotime($rs2_a[$iii]['听课日期'])+3*24*3600);
		$sql = "update td_edu.edu_tingke set 上课打卡时间 = '$时间' ,上课考勤状态='正常刷卡',最迟填写时间 = '$lll' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
		//$returnText .= "$教师姓名 上课正常 $考勤日期 $时间<BR>";
		global $SHOWTEXT; if($SHOWTEXT)print "定义正常打卡上课:".$sql."<BR>";
	...
	$dateTEMP = date("Y-m-d");
	$sql = "update td_edu.edu_tingke set 上课打卡时间='上课缺打卡',上课考勤状态='上课缺打卡' where 听课日期<'$dateTEMP' and (上课打卡时间='' or 上课打卡时间='上课缺勤')";
	$db->Execute($sql);
	//同步教师听课考勤数据结束（上课）
	//###########################################################################
	return $returnText;
...
function DealTimeJieCiXiaKe($教师姓名,$教师用户名,$考勤日期,$时间)		{
	global $db,$InsertData;
	$sql = "select * from td_edu.edu_teacherkaoqinmingxi where 教师用户名='$教师用户名' and 考勤日期='$考勤日期' and 下课刷卡BGN<='$时间' and 下课刷卡END>='$时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	$returnText = "";
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		$编号 = $rs2_a[$iii]['编号'];
		$sql = "update td_edu.edu_teacherkaoqinmingxi set 下课实际刷卡 = '$时间' ,下课考勤状态='正常刷卡' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
	...
	//###########################################################################
	//同步教师听课考勤数据开始（下课）
	$sql = "select * from td_edu.edu_tingke where 听课教师姓名='$教师姓名' and 听课日期='$考勤日期' and 下课刷卡BGN<='$时间' and 下课刷卡END>='$时间'";
	$rs2 = $db->Execute($sql);
	$rs2_a = $rs2->GetArray();
	$returnText = "";
	for($iii=0;$iii<sizeof($rs2_a);$iii++)						{
		$编号 = $rs2_a[$iii]['编号'];
		$lll=date("Y-m-d",strtotime($rs2_a[$iii]['听课日期'])+3*24*3600);
		$sql = "update td_edu.edu_tingke set 下课打卡时间 = '$时间' ,下课考勤状态='正常刷卡',最迟填写时间 = '$lll' where 编号='$编号'";
		$db->Execute($sql);
		$InsertData++;
		//$returnText .= "$教师姓名 下课正常 $考勤日期 $时间<BR>";
		global $SHOWTEXT; if($SHOWTEXT)print "定义正常打卡下课:".$sql."<BR>";
	...
	$dateTEMP = date("Y-m-d");
	$sql = "update td_edu.edu_tingke set 下课打卡时间='下课缺打卡',下课考勤状态='下课缺打卡' where 听课日期<'$dateTEMP' and (下课打卡时间='' or 下课打卡时间='下课缺勤')";
	$db->Execute($sql);
	//同步教师听课考勤数据结束（下课）
	//###########################################################################
	return $returnText;
